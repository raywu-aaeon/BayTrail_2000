...
*///////////////////////////////////////////////////////
        ...
        wpa_printf(MSG_DEBUG, "PKCS7: digestAlgorithm SHA1/SHA256 "
               "expected");
        asn1_oid_to_str(&oid, sbuf, sizeof(sbuf));
...
*/
        ...
        wpa_printf(MSG_DEBUG, "PKCS7: hashAlgorithm SHA1/SHA256 "
               "expected");
        asn1_oid_to_str(&oid, sbuf, sizeof(sbuf));
...
// process tstInfo
    ...
    if (asn1_get_next(pos, end - pos, &hdr) < 0 ||
        hdr.class != ASN1_CLASS_CONTEXT_SPECIFIC) {
        wpa_printf(MSG_DEBUG, "X.509: Failed to parse "
               "AttributeValue");
        // pkcs7_signed_data_free(SignedData);
        return NULL;
    }
...
//         os_free(data);
    ...
    if (x509_sha1_oid(&oid)) {
        if(!x509_sha1_oid(digest_alg)) {
            wpa_printf(MSG_DEBUG, "PKCS7: digestAlgorithm SHA1 "
...
//              os_free(data);
        ...
        if (x509_sha256_oid(&oid)) {
            if(!x509_sha256_oid(digest_alg)) {
                wpa_printf(MSG_DEBUG, "PKCS7: digestAlgorithm SHA256 "
                       "does not match with certificate "
                       "signatureAlgorithm (%x)",
                       digest_alg->oid[6]);
//              crypto_public_key_free(pk);
...
{
    u8 bSha1 = FALSE;//, bSha256 = FALSE;
    size_t hashLen = 0;
    u8 hash[SHA256_MAC_LEN];    // calc hash on auth attrib
    u8 *pHash = &hash[0];
    int err = -1;
        ...
        os_time_t *TimeOfSigning)
{
    ...
    if(SignedData->tstSignedData && SignedData->/*tstSignedData->*/tstInfo) {
        ...
        if (x509_sha1_oid(&SignedData->tstInfo->digest_alg.oid)) {
            hashLen = 20;
            sha1_vector(1, &SignedData->SignerInfo->enc_digest, &SignedData->SignerInfo->enc_digest_len, hash);
        } else 
            if (x509_sha256_oid(&SignedData->tstInfo->digest_alg.oid)) {
                hashLen = 32;
                sha256_vector(1, &SignedData->SignerInfo->enc_digest, &SignedData->SignerInfo->enc_digest_len, hash);
            } else    {
                wpa_printf(MSG_DEBUG, "PKCS7: Unsupported digestAlgorithm");
                return err;
            }
