...
{
	struct asn1_hdr hdr;
	const u8 *s, *pos, *end, *set_pos, *set_end, *seq_pos, *seq_end;
	struct asn1_oid oid;
	char **fieldp;
	int i;
	char *d;
...
//			x509_free_name(name);
		...
		os_free(*fieldp);
		*fieldp = (char*)os_malloc(hdr.length + 1);
		if (*fieldp == NULL) {
//			x509_free_name(name);
			return -1;
		}
//*** AMI W/A ***//
// remove Zeroes in unicode provided strings and compress the output string
			       ...
			       struct x509_certificate *cert, const u8 **next)
{
	...
	*next = pos + plen;
	...
	wpa_printf(MSG_MSGDUMP, "X509: Validity: notBefore: %x notAfter: %x",
				      ...
				      const u8 **next)
{
	...
	wpa_printf(MSG_MSGDUMP, "X509: serialNumber %x", cert->serial_number);
...
/*
	...
	cert->cert_len = len;
*/
	cert = (struct x509_certificate *)os_zalloc(sizeof(*cert));
	os_memset(cert, 0, sizeof(*cert));	
	if (cert == NULL)
		return NULL;
	cert->cert_start = (u8 *) (buf);
//	cert->cert_len = len;
/////////////////////////////////////////////////
	pos = buf;
	end = buf + len;
...
//		x509_certificate_free(cert);
	...
	pos = hdr.payload;
	//AMI W/A. extract true cert size from ASN1 header
    cert->cert_len = hdr.length + (pos-buf);
...
//	os_free(cert->sign_value);
//	cert->sign_value = (u8*)os_malloc(hdr.length - 1);
...
/* Hash field must be at the right of the decoded Sig data*/
	if((hdr.payload + hdr.length) < end){
		wpa_printf(MSG_DEBUG, "X509: Pkcs#1v1.5 Signature invalid");
		os_free(data);
		return -1;
	}
/* AMI FIX */		
				    ...
				    int *reason)
{
	...
	*reason = X509_VALIDATE_OK;
	...
	for (cert = chain, idx = 0; cert; cert = cert->next, idx++) {
		...
		wpa_printf(MSG_DEBUG, "X509: %x: %s", idx, buf);
		...
		if ((unsigned long) now.sec <
			...
			wpa_printf(MSG_INFO, "X509: Certificate not valid "
				   "(now=%x not_before=%x not_after=%x)",
...
#endif /*undefined  CONFIG_NO_INTERNAL_PEI_PKCS7 */
