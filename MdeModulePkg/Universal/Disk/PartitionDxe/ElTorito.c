...
//*** AMI PORTING BEGIN ***//
      if(PcdGetBool(PcdUdfSupport) || PcdGetBool(PcdIso9660Support)) {
    	  SaveVolDescriptor = AllocatePool ((UINTN) Media->BlockSize);
    	  if(SaveVolDescriptor != NULL)
    		  CopyMem(SaveVolDescriptor, VolDescriptor, (UINTN) Media->BlockSize);
      }
//*** AMI PORTING END ***//
...
//*** AMI PORTING BEGIN ***//
  ...
  // ISO9660 Filesystem support
        if (EFI_ERROR(Status) && PcdGetBool(PcdIso9660Support)){
