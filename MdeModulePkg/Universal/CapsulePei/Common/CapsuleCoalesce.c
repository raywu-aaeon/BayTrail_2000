  ...
  )
{
  ...
  while (Desc->Union.ContinuationPointer != (EFI_PHYSICAL_ADDRESS) (UINTN) NULL) {
    if (Desc->Length == 0) {
      ...
      //
	  //*** AMI PORTING BEGIN ***//
	  // Validate the continuation pointer:
	  // 1. EIP(107134) When continuation pointer is the same as the current pointer,
	  // the capsule is corrupted.
	  // 2. The pointer can't be greater than MAX_ADDRESS
