...
#include "Capsule.h"
//*** AMI PORTING BEGIN ***//
// We are using CapsuleLib to process a capsule based on its type
// See ProcessCapsuleImage call in CreateState
  ...
  )
{
  ...
  EFI_STATUS                           Status;
  //*** AMI PORTING BEGIN ***//
  //The variable is not used. See comment below. 
  ...
  //
  //*** AMI PORTING BEGIN ***//
  // Original code skips capsule check if boot mode is not set to BOOT_ON_FLASH_UPDATE.
  // This is questionable. Not every capsule is a flash update capsule.
  /*
  ...
  */
  ...
  if (FeaturePcdGet (PcdDxeIplSwitchToLongMode)) {
    ...
    if (EFI_ERROR (Status)) {
	  ...
	  //*** AMI PORTING BEGIN ***//
	  // If we can't switch to long mode, continue in the 32-bit mode
      //Status = EFI_NOT_FOUND;
      //goto Done;
      goto TryIA32Mode;
	  //*** AMI PORTING END *****//
  ...
  )
{
  ...
  for (Index = 0; Index < CapsuleNumber; Index ++) {
    ...
    //*** AMI PORTING BEGIN ***//
    // Perform additional processing of the capsule.
	ProcessCapsuleImage((EFI_CAPSULE_HEADER *)((UINTN) BaseAddress));
    //*** AMI PORTING END *****//  	
  }
