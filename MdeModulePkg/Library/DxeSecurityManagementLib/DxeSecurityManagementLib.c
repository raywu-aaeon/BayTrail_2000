...
//*** AMI PORTING BEGIN ***//
// The security handlers that deal with the image shouldn't use this function. 
// They should use RegisterSecurity2Handler instead. 
// However, an older handlers created when RegisterSecurity2Handler didn't exist 
// may still use this function.
// The update of these older handlers may be undesirable, 
// because it makes them incompatible with the older versions of the Core.
// The code below detects such handlers and redirects them to RegisterSecurity2Handler.
  if (    (AuthenticationOperation & EFI_AUTH_OPERATION_AUTHENTICATION_STATE)
       != EFI_AUTH_OPERATION_AUTHENTICATION_STATE
  ) return RegisterSecurity2Handler(
     (SECURITY2_FILE_AUTHENTICATION_HANDLER)SecurityHandler,
...
{ 
  ...
  //
//*** AMI PORTING BEGIN ***//
// We are disabling the check below, because we can handle this situation.
// See comments in RegisterSecurity2Handler for additional details.
/*
  if ((CurrentAuthOperation & EFI_AUTH_OPERATION_MEASURE_IMAGE) == EFI_AUTH_OPERATION_MEASURE_IMAGE) {
    if (((CheckAuthOperation & EFI_AUTH_OPERATION_MEASURE_IMAGE) == EFI_AUTH_OPERATION_MEASURE_IMAGE) ||
...
//*** AMI PORTING BEGIN ***//
// See comments below.
  static UINT32 NumberOfMeasuringHandlers = 0;
  UINT32 i;
//*** AMI PORTING END *****//
  EFI_STATUS  Status;
  ...
  //
//*** AMI PORTING BEGIN ***//  
// The measuring handlers must be at the end of the list.
// The code below enforces this policy.
// The original code was generating an assert in the CheckAuthentication2Operation.
/*
...
*/
  if (  (AuthenticationOperation & EFI_AUTH_OPERATION_MEASURE_IMAGE)
      !=EFI_AUTH_OPERATION_MEASURE_IMAGE
  ){
    // The handler we are registering is not the measuring one.
	// We have to stick it before all the measuring handlers.
	// The for-loop below shifts all the measuring handlers down creating an opening
	// for the new handler.
    for(i=0; i<NumberOfMeasuringHandlers; i++){
      mSecurity2Table[mNumberOfSecurity2Handler-i] = mSecurity2Table[mNumberOfSecurity2Handler-i-1];
    }
  }else{
    i=0; NumberOfMeasuringHandlers++;  
  }
  mSecurity2Table[mNumberOfSecurity2Handler-i].Security2Operation = AuthenticationOperation;
  mSecurity2Table[mNumberOfSecurity2Handler-i].Security2Handler   = Security2Handler;
  mNumberOfSecurity2Handler ++;
//*** AMI PORTING END *****//
