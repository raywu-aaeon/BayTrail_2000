  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // In Aptio4 some modules were using EfiReservedMemoryType.
  // Add debug message to explain why the code is asserting.
    DEBUG ((EFI_D_ERROR, 
		"ERROR: SmmAllocatePool: Attempt to allocate a pool with invalid type %d.\n"
		"Only EfiRuntimeServicesCode or EfiRuntimeServicesData are allowed\n"
		"(see SmmAllocatePool() description in the Volume 4 of the PI spec).\n"
		"PLEASE UPDATE YOUR CODE TO USE PROPER POOL TYPE!!!\n",
		PoolType
	));
  //*** AMI PORTING END *****//  		  	  		
