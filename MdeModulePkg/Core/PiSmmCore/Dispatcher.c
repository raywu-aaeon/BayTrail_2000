...
// Move debug message
  ...
  DEBUG_CODE_BEGIN ();
	UINTN length;
    CHAR8 *temp;
    CHAR8 *PdbPointer = PeCoffLoaderGetPdbPointer(DriverEntry->LoadedImage->ImageBase);
    if (PdbPointer != NULL) {
        length = AsciiStrLen(PdbPointer);
        if (length>4 && !AsciiStrCmp(&PdbPointer[length-4],".pdb")) {
            for(temp = &PdbPointer[length-5]; temp>=PdbPointer && *temp!='\\'; temp--);
            temp++;
