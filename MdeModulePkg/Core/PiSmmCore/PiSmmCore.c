...
//
//*** AMI PORTING BEGIN ***//
// The comment right above is not true for Aptio.
// Aptio CSM converts runtime code and data into reserved ranges in the E820 memory map.
// Legacy ACPI-compliant OS shouldn't touch these ranges.
// The mInLegacyBoot variable below is set to TRUE by SmmLegacyBootHandler.
// Once this happens, the SmmCommunicate no longer works.
// To enable SmmCommunicate on legacy boot, we are disabling legacy boot even handler
// (one line is commented below; as a result SmmLegacyBootHandler is never called).
//*** AMI PORTING END *****//
