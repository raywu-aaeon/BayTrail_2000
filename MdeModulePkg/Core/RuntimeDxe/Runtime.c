  ...
  )
{
  ...
  for (Link = mRuntime.EventHead.ForwardLink; Link != &mRuntime.EventHead; Link = Link->ForwardLink) {
    ...
    if ((RuntimeEvent->Type & EVT_SIGNAL_VIRTUAL_ADDRESS_CHANGE) == EVT_SIGNAL_VIRTUAL_ADDRESS_CHANGE) {
      //*** AMI PORTING BEGIN ***
      // Some buggy boot time drivers register virtual address change callbacks.
      // The code below checks if callback function really belongs to a runtime driver.
      // If not, the function is not called. In debug mode this will trigger an assert.
      VOID *VirtualizedNotifyFunction = RuntimeEvent->NotifyFunction;
      if (EFI_ERROR(RuntimeDriverConvertInternalPointer(&VirtualizedNotifyFunction))){
            DEBUG((EFI_D_ERROR,"ERROR: Virtual address change callback at address %X resides in the boot time memory\n",RuntimeEvent->NotifyFunction));
            ASSERT(FALSE);
            continue;
      }
      //*** AMI PORTING END *****
...
//*** AMI PORTING BEGIN ***//
  // Report Runtime Set Virtual Address MAP End
  POST_CODE(0xB1);
//*** AMI PORTING END *****//
