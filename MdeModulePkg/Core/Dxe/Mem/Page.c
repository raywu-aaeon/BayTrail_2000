...
EFI_MEMORY_TYPE_STATISTICS mMemoryTypeStatistics[EfiMaxMemoryType + 1] = {
  ...
  //*** AMI PORTING BEGIN ***//
  // This is a workaround for a Windows 8 bug.
  // Windows 8 expects that memory map does not change after the boot loader launch.
  // However, in certain cases Windows 8 calls ConnectController for all the devices,
  // which may cause quite a lot of EfiBootServicesData memory allocations.
  // Changing the "Special" field to TRUE freezes the EfiBootServicesData entries in the memory map.
  // { 0, EFI_MAX_ADDRESS, 0, 0, EfiMaxMemoryType, FALSE, FALSE },  // EfiBootServicesData
  { 0, EFI_MAX_ADDRESS, 0, 0, EfiMaxMemoryType, TRUE, FALSE },  // EfiBootServicesData
  //*** AMI PORTING END *****//
...
EFI_PHYSICAL_ADDRESS mDefaultBaseAddress = EFI_MAX_ADDRESS;
  ...
  )
{
  ...
  // This change is required by UEFI mantis ticket 909, which is part of UEFI 2.3.1C.
  ...
  )
{
  ...
  for (Link = gMemoryMap.ForwardLink; Link != &gMemoryMap; Link = Link->ForwardLink) {
    ...
    //*** AMI PORTING BEGIN ***//
    // Bug fix: The operands of the comparison operation below are enum types.
    // According to C standard, enum may be signed or unsigned (implementation specific).
    // If enum is signed (like in VC compiler), and MemoryMap->Type value
    // is in the range 0x80000000..0xFFFFFFFF (reserved by UEFI spec. for use by OS loaders),
    // the result of the original comparison is TRUE, which casues our-of-range access of the mMemoryTypeStatistics array.
