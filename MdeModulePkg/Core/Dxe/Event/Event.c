  ...
  )
{
  ...
  for(;;) {
    ...
    //
//*** AMI PORTING BEGIN ***//
// We are disabling triggering on the idle event.
// The problem is that a CPU module typically installs a callback on this event 
// that sends CPU to a sleep state (hlt instruction on Intel Architecture). 
// CPU becomes unconscious until the next interrupt. 
// This drastically changes the nature of the WaitForEvent function.
// According to the UEFI specification (UEFI 2.3.1C, Page 124):
// "The list of events in the Event array are evaluated in order from first to last,
//  and this evaluation is repeated until an event is signaled or an error is detected."
...
// a polling function into a evaluate-once-and-wait-for-an-interrupt function, 
// which, given a non-interrupt driven nature of the UEFI firmware, 
// can lead to unpleasant side effects. 
// In a typical UEFI implementation where only timer interrupt is enabled, 
// the events will be evaluated once per timer interrupt cycle, 
// which is quite different from the repetitive evaluation mandated by the UEFI specification.
//
// Some may argue that rather than disabling triggering of the idle event, 
// it is better to disable the ill-behaved handlers.
// However, even well-behaved idle event handlers are stealing CPU cycles from the handlers 
// of the events we are waiting on thus making WiatForEvent less effective. 
// Besides, there are multiple idle handlers out there. Some of them are in the code
// not controlled by AMI. It’s quite difficult to change all of them.
//
// Technical details: 
// Here are some of the CPU modules that install an idle event callback:
// ArmPkg\Drivers\CpuDxe, IA32FamilyCpuPkg\CpuMpDxe,
// UefiCpuPkg\CpuDxe, EmulatorPkg\CpuRuntimeDxe
// They are sending CPU into a sleep state using CpuSleep function from a CpuLib library class.
// Typical project uses this instance: MdePkg\Library\BaseCpuLib\BaseCpuLib.inf
