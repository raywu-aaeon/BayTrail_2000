...
#include "DxeMain.h"
//*** AMI PORTING BEGIN ***//
#include <AmiStatusCodes.h> // To report missing arch. protocol error
#include <AmiDxeLib.h>	//	Adding FPDT support.
#include <DxeCoreHooks.h> // DxeCoreInitialize eLink support
//*** AMI PORTING END *****//
...
extern INIT_FUNCTION DXE_CORE_INITIALIZE_HOOKS EndOfInitList;
INIT_FUNCTION* DxeCoreInitializeList[] = {DXE_CORE_INITIALIZE_HOOKS NULL};

VOID ProcessDxeCoreInitializeHooks (
  IN EFI_HANDLE        ImageHandle,
  IN EFI_SYSTEM_TABLE  *SystemTable
){
  UINTN i;
  for(i=0; DxeCoreInitializeList[i]; i++) DxeCoreInitializeList[i](ImageHandle,SystemTable);
}
//*** AMI PORTING END *****//
  ...
  )
{
  ...
  //*** AMI PORTING BEGIN ***//
  // This is required to be able to use other AmiDxeLib functions.
  InitAmiLib(gDxeCoreImageHandle,gDxeCoreST);
  //*** AMI PORTING END ***//
  ...
  //*** AMI PORTING BEGIN ***//
  PERF_START (NULL,"DxeMainInit", "DxeMain", 0) ;
  //*** AMI PORTING END *****//  
  ...
  // Stop the DxeMainInit measurement
  PERF_END (NULL,"DxeMainInit", "DxeMain", 0) ;
  //*** AMI PORTING END *****//    
...
// 1. According to ExitBootServices() description in the UEFI specification (startinf from version 2.2)
//    the EXIT_BOOT_SERVICES handlers must be called once even if ExitBootServices() is called more than once.
// 2. Terminate memory map after execution of the EXIT_BOOT_SERVICES handlers.
//    Otherwise memory allocations within the handlers will not be detected during map
//    key validation performed by CoreTerminateMemoryMap.
// 3. ACPI FPDT table support (fill the exit boot services timings).
    VOID VeiryMemoryMapKey(IN UINTN MapKey);
    static BOOLEAN FirstCall = TRUE;
    // VeiryMemoryMapKey verifies MapKey is records verification results.
    // If MapKey is invalid, CoreTerminateMemoryMap call below will fail.
    VeiryMemoryMapKey(MapKey);

//Only call NotifySignalList() if this is first call to ExitBootServices()
  if (FirstCall) {
...
//*** AMI PORTING BEGIN ***//
// Already called CoreNotifySignalList() above
  //
  // Notify other drivers that we are exiting boot services.
  //
  //CoreNotifySignalList (&gEfiEventExitBootServicesGuid);
