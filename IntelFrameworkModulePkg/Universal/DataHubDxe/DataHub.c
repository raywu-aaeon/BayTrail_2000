  ...
  )
{
  ...
  //
  //*** AMI PORTING BEGIN ***//
  // The GetNextHighMonotonicCount call below is not needed.
  // The MTC increment on every boot is responsibility of the MonotonicCounter driver
  // (see GetNextHighMonotonicCount() description in UEFI spec).
  // Because of this call, the MTC counter is increased by 2 on every boot.
  // The data hub driver should use GetNextMonoticCount(). 
  // It provides unique MTS and does not unnecessary increment the non-volatile part of the counter.

  // Status = gRT->GetNextHighMonotonicCount (&HighMontonicCount);
  Status = gBS->GetNextMonotonicCount(&mPrivateData.GlobalMonotonicCount);
  //*** AMI PORTING END *****//  
  if (EFI_ERROR (Status)) {
    //
