  ...
  )
{
  ...
  //  Display entry
  //
  DEBUG (( DEBUG_INFO, "I2cHostDriverStart entered\r\n" ));
  ...
  if ( !EFI_ERROR ( Status )) {
    ...
    //
    //  Release the I2C master protocol upon failure
    //
    if ( EFI_ERROR ( Status )) {
  ...
  //  Display exit
  //
  DEBUG (( DEBUG_INFO, "I2cHostDriverStart exiting, Status: %r\r\n", Status ));
  ...
  )
{
  ...
  if ( !EFI_ERROR ( Status )) {
    ...
    if ( !EFI_ERROR ( Status )) {
      ...
      if ( !EFI_ERROR ( Status )) {
        ...
        //
        //  Release the I2C bus configuration management protocol if necessary
        //
        if (( !EFI_ERROR ( Status ))
            && ( NULL != I2cHost->I2cBusConfigurationManagement )) {
          Status = gBS->CloseProtocol ( Controller,
                          &gEfiI2cBusConfigurationManagementProtocolGuid,
                          DriverBinding->DriverBindingHandle,
                          Controller );
  ...
  //  Display exit
  //
  DEBUG (( DEBUG_INFO, "I2cHostDriverStop exiting, Status: %r\r\n", Status ));
