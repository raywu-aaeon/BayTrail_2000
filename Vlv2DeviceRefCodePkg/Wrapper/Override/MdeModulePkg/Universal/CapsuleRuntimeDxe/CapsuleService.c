  ...
  )
{
  ...
  for (ArrayNumber = 0; ArrayNumber < CapsuleCount ; ArrayNumber++) {
    ...
    if ((CapsuleHeader->Flags & CAPSULE_FLAGS_PERSIST_ACROSS_RESET) == 0) {
      ...
      //
      // If it is AOS SystemFirmwareGuid, then call ProcessCapsuleImage
      //
      if ((CompareGuid(&gAosFirmwareClassGuid, &CapsuleHeader->CapsuleGuid))){
        Size = sizeof (AosCapsule);
        ...
        if ((Status == EFI_SUCCESS) && (AosCapsule == 1)) {
          AosCapsule = 0;
