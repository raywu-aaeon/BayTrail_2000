...
//<AMI_PHDR_END> 
    Method (MXMX, 1, Serialized) 
    {
        ...
        If (LEqual (Arg0, 0)) 
        {
            ...
            // Set mux to iGPU
//            ASL_DGPUPCH_SCOPE.SGPO(DSEL, 0)
//            ASL_DGPUPCH_SCOPE.SGPO(ESEL, 0)  // use edid_select# as mutex flag
...
//            Return(LNot(ASL_DGPUPCH_SCOPE.SGPI(ESEL)))
        }
...
//            return(LNot(ASL_DGPUPCH_SCOPE.SGPI(DSEL)))
        } else       
//        If(LOr(LEqual (Arg0, 1), LEqual (Arg0, 2)))
        {
            //Set display mux to igpu
//            ASL_DGPUPCH_SCOPE.SGPO(DSEL, 0)
//            ASL_DGPUPCH_SCOPE.SGPO(PSEL, 0)
        }
...
//x     if(CMPB(Arg0, Buffer(){0xFD,0x88,0xDB,0x95,0x0A,0x94,0x53,0x42,0xA4,0x46,0x70,0xCE,0x05,0x04,0xAE,0xDF}))
        ...
        {
            return ( ASL_DGPUPCH_SCOPE.SPB(Arg0, Arg1, Arg2, Arg3))
        }
#endif
...
//x     if(CMPB(Arg0, Buffer(){0x01, 0x2D, 0x13, 0xA3, 0xDA, 0x8C, 0xBA, 0x49,0xA5, 0x2E, 0xBC, 0x9D, 0x46, 0xDF, 0x6B, 0x81}))
        ...
        {
            return ( ASL_DGPUPCH_SCOPE.GPS(Arg0, Arg1, Arg2, Arg3))
        }
#endif

#if (ASL_NV_GC6_SUPPORT == 1) || (ASL_NV_GC6_SUPPORT == 2)
        If(LEqual(Arg0, ToUUID("CBECA351-067B-4924-9CBD-B46B00B86F34")))
        {
            return ( ASL_DGPUPCH_SCOPE.NGC6(Arg0, Arg1, Arg2, Arg3))                 
        }
#endif
...
//                            Return(ToBuffer(0x0c010001))
            ...
            If(LEqual(Arg2,2))
            {
                ...
                If(And(Local0,0x10))    //If Switchable policy update bit is set
                {
                    ...
                    Store(Local0,GPSS)
                    Notify(ASL_IGPU_SCOPE,0xD9)     //Broadcast "policy completed" notification
                    Notify(ASL_PCI_SCOPE.WMI1, 0xD9)    // Mirror Notify on WMI1
            ...
            If(LEqual(Arg2,3))
            {
                ...
                If(LEqual(Local0,0))
                {
                    ASL_DGPUPCH_SCOPE.SGST()
                }
    
                If(LEqual(Local0,1))
                {
                    ASL_DGPUPCH_SCOPE.HGON()
                }
    
                If(LEqual(Local0,2))
                {
                    ASL_DGPUPCH_SCOPE.HGOF()
                }
    
                //dGPU_PWROK is not working. Using dGPU_PWR_EN# instead as w/a
                //Or(STS2,ASL_DGPUPCH_SCOPE.MPOK,STS2)
                If(LEqual(ASL_DGPUPCH_SCOPE.SGST(), 0xF))
                {
                    Or(STS2,0x1,STS2)
                }
...
// common for SGCI and NBCI
		...
		CreateField(Arg3,0x02,One,DVSC)
                If(ToInteger(FLCH))
                {
                    Store(OMPR, ASL_DGPUPCH_SCOPE.OPCE)                   // Optimus Power Control Enable - From DD
                }
                // On return
		...
		If(ToInteger(DVSC))
		{
			If(ToInteger(DVSR))
			{
				Store(One, ASL_DGPUPCH_SCOPE.GPRF)
			}
			Else
			{
				Store(Zero, ASL_DGPUPCH_SCOPE.GPRF)
			}
                } 
		Store(ASL_DGPUPCH_SCOPE.GPRF, SNSR)


	 
              If(LNotEqual(ASL_DGPUPCH_SCOPE.SGST(), 0))
                {
...
//                Store(ASL_SWSMI_NVOEM_CMOS_W, SSMP)  // Set Audio Codec flag to CMOS
                Return(Local0)
            }
...
//      return (ASL_DGPUPCH_SCOPE.HDSM(Arg0, Arg1, Arg2, Arg3))
      Return (0x80000001)
    }
} // end PCI0.GFX0 scope
