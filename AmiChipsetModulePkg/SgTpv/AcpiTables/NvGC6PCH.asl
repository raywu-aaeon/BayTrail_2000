...
External(ASL_SG_ULT_RP_NUM.LNKS)
  ...
  OperationRegion(ECF3,EmbeddedControl,0,0xFF)
	...
	Method(_Q60, 0, NotSerialized) // for GC6 entry Q-event
	{
		Store("------- GC6I-SCI _Q event --------", Debug)
                CreateField(ASL_DGPUPCH_SCOPE.TGPC,0x0A,0x2,PRGE)                // PRGE : PCIe Root Power GC6 Enter Sequence
                If(LEqual(ToInteger(PRGE), 0x0))                                   // DAGC : Link Disable after GC6 Entry complete & before GPU Power Down
                {
			Store(One, ASL_SG_ULT_RP_NUM.LNKD)                           // PCIE link disabling.
        	}
        		
//		ASL_DGPUPCH_SCOPE.SGPO(HLRS, 0) // dGPU RST# to low
//		ASL_DGPUPCH_SCOPE.SGPO(PWEN, 0)  // dGPU PWN Enable to low
...
Scope (ASL_DGPUPCH_SCOPE)
{
  Name(TGPC, Buffer(0x04)
  {
   ...
   Method(GC6O, 0, Serialized)
   {
                ...
                {
		Store(Zero, ASL_SG_ULT_RP_NUM.LNKD)          //  PCIE link enabling
                }
//		ASL_DGPUPCH_SCOPE.SGPO(HLRS, 0)	   //  dGPU RST# is low 
//         	ASL_DGPUPCH_SCOPE.SGPO(PWEN, 1)	   //  dGPU PWR Enable is high 
...
//		ASL_DGPUPCH_SCOPE.SGPO(HLRS, 1)	   //  dGPU RST# is high
		...
		// Haswell UTL has no LNKS register, please remove it from Haswell platform
                While(LLess(ASL_SG_ULT_RP_NUM.LNKS, 0x07))
		{
			Sleep(One)
		}
   ...
   {
		If(LEqual(ASL_DGPUPCH_SCOPE.SGPI(PWEN), One))   // dGPU PWR Enable is high
		{
