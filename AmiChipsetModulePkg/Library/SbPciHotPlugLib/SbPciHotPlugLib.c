...
//<AMI_FHDR_START>
//
// Name:  SbPciHotPlugLib.c
//
// Description: Library Class for AMI PCI Hotplug function.
//
//
//<AMI_FHDR_END>
//*************************************************************************
...
BOOLEAN HpcCheckHpCompatibleHost(PCI_HOST_BRG_DATA *HostBrg){
    ...
    if(HostBrg->HpcData == NULL)
        PCI_TRACE((TRACE_PCI,"Barret: HostBrg->HpcData is NULL\n"));
    
    if(HostBrg->HpcData->HpcLocCount > 0){
        PCI_TRACE((TRACE_PCI,"Barret: HpcLocCount is %d\n", HostBrg->HpcData->HpcLocCount));
        return TRUE;
    }
    
    PCI_TRACE((TRACE_PCI,"Barret: HpcCheckHpCompatibleHost function return FALSE\n"));
	return FALSE;
}
...
//  EFI_OUT_OF_RESOURCES    When system runs out of resources.
//  EFI_DEVICE_ERROR        When driver fails access PCI Bus.
//  Barret Keep and Change to our style
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
...
//-------------------------------------
    ...
    PCI_TRACE((TRACE_PCI,"Barret: PciDP Dev:%d Fun:%d\n", pcidp.Device, pcidp.Function));
    ...
    PCI_TRACE((TRACE_PCI,"Barret: Update get Padding Setup Data in HpcUpdateLocationData function\n"));
    ...
    if (parent->PciDevFlags.Bits.FixedBus){
        if (parent->Bus == 0 && pcidp.Device == 28){
            locdata->HpSetup.ArrayField.BusPadd      = gSetupData.ExtraBusRsvd[pcidp.Function];
            locdata->HpSetup.ArrayField.IoPadd       = gSetupData.PcieIoRsvd[pcidp.Function] * 0x400;
            locdata->HpSetup.ArrayField.Mmio32Padd   = gSetupData.PcieMemRsvd[pcidp.Function] * 0x100000;
            locdata->HpSetup.ArrayField.Mmio32PfPadd = gSetupData.PciePFMemRsvd[pcidp.Function] * 0x100000;
        }
    }
    //while(1); //Barret_ASSERT
...
//-------------------------
	...
	PCI_TRACE((TRACE_PCI,"---------------------------------------------------------------\n"));
    PCI_TRACE((TRACE_PCI,"PciHp: Looking for HP Bridges of RB '%s'; SdlIndex 0x%X(%d)...\n",
    		RootBrg->RbAslName,RootBrg->RbSdlIndex,RootBrg->RbSdlIndex));
    
    // Get Setup Items Data
    Status = pRS->GetVariable( L"Setup", \
                               &gEfiSetupGuid, \
                               NULL, \
                               &VariableSize, \
    ...
    for(i=0,hpbcnt=0; i<gSdlPciData->PciDevCount; i++){
    	...
    	if(sdltmpParent->PciDevFlags.Bits.RootBridgeType && sdltmpParent->PciDevFlags.Bits.FixedBus){
    	    PCI_TRACE((TRACE_PCI,"Barret: Parent = %s in SDL\n", sdltmpParent->AslName));
    	    if (sdltmpParent->Bus == 0 && sdltmp->Device == 28){
    	        PCI_TRACE((TRACE_PCI,"Barret: make sure Pci device is SB PCIE root port %s\n", sdltmp->AslName));
    	        sdltmp->PciDevFlags.Bits.HotPlug = gSetupData.PcieRootPortHPE[sdltmp->Function];
    	    }    
    	}
    		...
    		if(AmiSdlCheckIfParent(i, RootBrg->RbSdlIndex)){
    			...
    			for(j=0,br=TRUE,sltcnt=0; j<hpscnt; j++){
    				...
    				if(sdltmp->PciDevFlags.Bits.OnBoard){
    					...
    					PCI_TRACE((TRACE_PCI,"\tFound Slot# 0x%X(%d), SdlIndex 0x%X(%d) @ B:%d|D:%d|F:%d; HpDisable=%d; ",
...
//------------------------
    ...
    *HpcList=hpcl;
    PCI_TRACE((TRACE_PCI,"Barret: HpcInitGetRootHpcList return %r\n", Status));
...
// Change this call to outside lib OEM Porting function through Platform Init Protocol.
...
//------------------------
    ...
    PCI_TRACE((TRACE_PCI,"Barret: HpcGetResourcePadding Entry !!!\n"));
    ...
    PCI_TRACE((TRACE_PCI,"Barret: Hotplug location: %x -> B:%x|D:%x|F:%x\n", HpcPciAddress, Bus, Dev, Fun));
    
    //Find which exactly HPC PCI BUS Driver means..
    Status=FindHpcLocData(HpcDevicePath, hpcdata,&locdata);
    PCI_TRACE((TRACE_PCI,"Barret: Get Hotplug resource information for this device %r\n", Status));
    if(EFI_ERROR(Status)) return Status;
    ...
    PCI_TRACE((TRACE_PCI,"Barret: Calculate how many ACPI_QW_DESC we need for Padding Info. cnt = %x\n", cnt));
    ...
    PCI_TRACE((TRACE_PCI,"Barret: fill out QW Resopurce Descriptors for resource requirements.\n"));
    for(i=0, qw=pd; i<ptMaxType; i++){
    	if(i==ptIo32)continue;
        if(locdata->HpSetup.ARRAY[i]!=0){
            if (Bus == 0 && Dev == 28){  // SB PCIE root port
                // Get Setup Items Data first
                Status = pRS->GetVariable( L"Setup", \
                                           &gEfiSetupGuid, \
                                           NULL, \
                                           &VariableSize, \
                ...
                switch(i){
                    case ptMmio32:
                    case ptMmio64:
                        HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], gSetupData.PcieMemRsvdalig[Fun]);
                        break;
                    case ptMmio32pf:
                    case ptMmio64pf:
                        HpcFillDescriptor(qw, (HP_PADD_RES_TYPE)i, locdata->HpSetup.ARRAY[i], gSetupData.PciePFMemRsvdalig[Fun]);
                        break;
                    default:
            ...
            PCI_TRACE((TRACE_PCI,"Barret: resource type: %x. resource size: %x\n", i, locdata->HpSetup.ARRAY[i]));
            qw++;
        }
    }
