...
//Replace this with more generic code
    ...
    IN BOOLEAN                  Enable )
{
    ...
    if ((Length == 0) || LpcPciIoProtocol==NULL) return EFI_INVALID_PARAMETER;
...
//    WRITE_PCI32_SB(ICH_REG_LPC_GEN1_DEC + IoGenDecIndex * 4, IoGenDecode32); // 0x84
    Status=LpcPciIoProtocol->Pci.Read(LpcPciIoProtocol, EfiPciIoWidthUint32,(ICH_REG_LPC_GEN1_DEC + IoGenDecIndex * 4),1,&IoGenDecode32);
    ASSERT_EFI_ERROR(Status);
...
//-------------------------------------------------------------------------
// <AMI_PHDR_END>
EFI_STATUS SioLibLaunchCompRoutine(SIO_DEV2 *Device, SIO_INIT_STEP InitStep)
{
...
//------------------------
	...
	SIO_TRACE((TRACE_SIO,"GSIO: Calling InitProtocol->Function[%d](InitStep=%d, AmiSio=%X, LpcPciIo=%X)...",
...
VOID SioLib_BootScriptIO(UINT16 Reg16, UINT8 Data8, EFI_S3_SAVE_STATE_PROTOCOL *SaveState)
{
	UINT8              value=0;
...
VOID SioLib_BootScriptSioS3SaveTable(
		UINT16 idxReg,
		UINT16 dataReg,
...
VOID SioLib_BootScriptRTS3SaveTable(UINT16 Base, SIO_DEVICE_INIT_DATA  *Table, UINT8 Count, EFI_S3_SAVE_STATE_PROTOCOL *SaveState)
{
    UINT8   i;
    UINT8   Value8;
