...
//
// Description:	
//  VOID PeiTrace(IN UINTN Level, IN CONST EFI_PEI_SERVICES **ppPS,
...
//
// Description:	
//  VOID PeiPerfMeasure(IN CONST EFI_PEI_SERVICES **PeiServices, IN CHAR16 *Token,
...
//
// Description:	
//  VOID SaveS3PerformanceData(IN CONST EFI_PEI_SERVICES **PeiServices) saves the
...
VOID SaveS3PerformanceData(IN CONST EFI_PEI_SERVICES **PeiServices){
    ...
    Status = PeiGetVariable(
        PeiServices,
		L"PerfDataMemAddr", &gEfiGlobalVariableGuid,
...
EFI_STATUS PeiLoadFile(
    ...
    EFI_STATUS Status = (*PeiServices)->LocatePpi(
        PeiServices, 
        &gLocalEfiPeiLoadFilePpiGuid,0, &Dummy, &Loader
    );
    if ((EFI_ERROR(Status))) return Status;
    return 
#if PI_SPECIFICATION_VERSION < 0x00010000
