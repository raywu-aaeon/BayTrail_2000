...
{
	///// *****************  AMI TODO  ********************************\\\\\\
	///// temporary implementation - set to return the default narrow width
	if (Width==NULL) return EFI_INVALID_PARAMETER;
...
EFI_STATUS HiiLibGetGlyphInfo(
	...
	///// *****************  AMI TODO  ********************************\\\\\\
	///// temporary implementation - set to return the default width and height
	if (GlyphWidth!=NULL) *GlyphWidth = EFI_GLYPH_WIDTH;
	if (GlyphHeight!=NULL) *GlyphHeight = EFI_GLYPH_HEIGHT;
	return EFI_SUCCESS;
}
...
)
{
    ...
    if (EFI_ERROR (Status)) {
    	TRACE((-1,"gEfiHiiPackageListProtocolGuid protocol is not found\n"));
        return Status;
    }
    ...
    if (EFI_ERROR (Status)) {
    	TRACE((-1,"gEfiHiiDatabaseProtocolGuid protocol is not found\n"));
        return Status;
    }
    ...
    TRACE((-1,"NewPackageList status: %r\n",Status));
    return Status;
}
