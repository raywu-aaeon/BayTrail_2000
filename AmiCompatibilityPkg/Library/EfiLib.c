...
static EFI_GUID gLocalEfiPerformanceProtocolGuid = EFI_PERFORMANCE_PROTOCOL_GUID;
...
BOOLEAN GetImageName(EFI_HANDLE ImageHandle, CHAR8 *sName)
{
	...
	if (   !EFI_ERROR(pBS->HandleProtocol(ImageHandle,&gLocalEfiComponentName2ProtocolGuid,&pComponentName))
		&& !EFI_ERROR(pComponentName->GetDriverName(pComponentName,LANGUAGE_CODE_ENGLISH,&wsName))
#if EFI_SPECIFICATION_VERSION > 0x20000 
//Try UEFI 2.0 ComponentName protocol
       ||  !EFI_ERROR(pBS->HandleProtocol(ImageHandle,&gLocalEfiComponentNameProtocolGuid,&pComponentName))
...
)
{
    ...
    for( ; !(isEndNode(pDp) && pDp->SubType == END_ENTIRE_SUBTYPE); pDp = NEXT_NODE(pDp) ) {
        DevicePathSize += NODE_LENGTH(pDp);
...
)
{
	...
	Status = pBS->LocateProtocol(&gLocalEfiSmmBaseProtocolGuid, NULL, &pSmmBaseFramework);
	if (EFI_ERROR(Status)) return Status;
	...
	if (!InSmm) {
		...
		Status = pBS->HandleProtocol(
			ImageHandle, &gLocalEfiLoadedImageProtocolGuid, &LoadedImage);
		if (EFI_ERROR(Status)) return Status;
...
)
{
    ...
    InitAmiLib(ImageHandle,SystemTable);
	Status = pBS->LocateProtocol(&gLocalEfiSmmBase2ProtocolGuid, NULL, &pSmmBasePi);
...
static EFI_STATUS InitSmmCallback(IN EFI_EVENT Event, IN VOID *Context)
{
	...
	// Get this driver's image's FilePath
	Status = pBS->HandleProtocol(
		TheImageHandle, &gLocalEfiLoadedImageProtocolGuid, &LoadedImage
    );
	if (EFI_ERROR(Status)) return Status;
...
)
{
    ...
    InitAmiLib(ImageHandle,SystemTable);
	SmmBaseStatus = pBS->LocateProtocol(&gLocalEfiSmmBaseProtocolGuid, NULL, &pSmmBaseFramework);
    ...
    { 
        ...
        if (EFI_ERROR(SmmBaseStatus))
            Status = RegisterProtocolCallback(&gLocalEfiSmmBaseProtocolGuid, InitSmmCallback,
			    NULL, &SmmBaseEvent, &p
		    );
        else
...
)
{
    ...
    Status = pBS->LocateProtocol (&gLocalEfiPerformanceProtocolGuid, NULL, (VOID **) &Perf);
    if (EFI_ERROR (Status)) {
        return Status;
    }
...
VOID WriteBootToOsPerformanceData (){
  ...
  Status = pBS->LocateProtocol (
                  &gLocalEfiPerformanceProtocolGuid,
                  NULL,
                  &DrvPerf
                  );
  if (EFI_ERROR (Status)) {
...
//<AMI_PHDR_END>
//*************************************************************************
//Defined in UefiRuntimeLib TODO: reivew if functions are really equivalent
