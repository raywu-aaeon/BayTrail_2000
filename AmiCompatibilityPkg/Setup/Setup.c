...
DLIST AmiCallbackList;
DLIST AmiStringInitializationList;
DLIST AmiExtractConfigList;
DLIST AmiRouteConfigList;
...
VOID InitLanguages(EFI_HII_STRING_PROTOCOL *HiiString, EFI_HII_HANDLE HiiHandle)
{
    ...
    while(LangCodes[i] != '\0' && i < Size)
    {
        ...
        while( LangCodes[j] != ';' && LangCodes[j] != '\0' && (j < Size) )
            j++;
...
VOID SetupNotificationFunction(IN EFI_EVENT Event, IN VOID *Context)
{
    ...
    AMI_STRING_INITIALIZATION_ENTRY *TempLink = NULL;
    ...
    if(!ResourcesLoaded)
    {
        ...
        NewDp = DPAddNode(Dp, (EFI_DEVICE_PATH_PROTOCOL*)&FormSetDevicePathNode);
        ...
        if(!EFI_ERROR(Status))
        {
            ...
            for(NextLink = (AMI_STRING_INITIALIZATION_ENTRY*)(AmiStringInitializationList.pHead);
                (NextLink != NULL) && (TempLink = NextLink, NextLink = (AMI_STRING_INITIALIZATION_ENTRY*)NextLink->Link.pNext, TRUE) ;
                )
                TempLink->Function(FormData.HiiHandle);
...
EFI_STATUS EFIAPI SetupEntry(IN EFI_HANDLE ImageHandle, IN EFI_SYSTEM_TABLE *SystemTable)
{
    ...
    DListInit(&AmiStringInitializationList);
    DListInit(&AmiExtractConfigList);
    DListInit(&AmiRouteConfigList);

    if(!DoesVariableExist(LastLangCodes, &guidSetup))
    ...
    else
    {
        ...
        RegisterProtocolCallback(&guidSetupEnter, SetupNotificationFunction, NULL, &SetupEnterEvent, &pSetupRegistration);
        #endif
...
/*
    ...
    {
        Status = TempLink->Function(This, Request, Progress, Results);
        if(!EFI_ERROR(Status))
            break;
    }
*/
...
)
{
    ...
    {
        Status = TempLink->Function(This, Configuration, Progress);
        if(!EFI_ERROR(Status))
            break;
    }
    */
...
)
{
    ...
    {
        if((TempLink->QuestionId == KeyValue))
        {
            Status = TempLink->Function(&AmiCallbackParameters);
            if(Status != EFI_UNSUPPORTED)
                break;
        }
    }
