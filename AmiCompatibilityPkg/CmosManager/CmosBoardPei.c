...
//----------------------------------------------------------------------------
// 
// Procedure: ReadWriteStandardRange
//
// Description:
...
//----------------------------------------------------------------------------
// 
// Procedure: ReadWriteBank1Translated
//
// Description:
...
//----------------------------------------------------------------------------
// 
// Procedure: CmosMgrBatteryIsGood
//
// Description:
//  This function is used to determining whether or not the CMOS battery is
//  good.
//
// Input:   
//  IN EFI_PEI_SERVICES **PeiServices
    ...
    IN  EFI_PEI_SERVICES        **PeiServices )
{
    ...
    // In the PEI phase, it's assumed to be the first boot if the Read Only
    // PPI is not found or if the MonotonicCounter variable is not found.
    ...
    if (PeiServices != NULL){
        ...
        if (!EFI_ERROR(Status)){
            Status = ReadOnlyVariable->GetVariable( ReadOnlyVariable,
                L"MonotonicCounter", &GlobalVariableGuid, NULL, &Size, &Dummy );
        }
        if (EFI_ERROR(Status)){
    	    IsFirstBoot = TRUE;
        }
...
//
// Description:
//      This function returns TRUE to specify the platform is always
