  ...
  )
{
  ...
  *Progress = Request;
  ...
  //  
  ZeroMem (&Configuration, sizeof (TREE_CONFIGURATION));
  ...
  Configuration.RevokeTrust = PcdGetBool (PcdRevokeTrust);
  ...
  )
{
  ...
  *Progress = Configuration;
  ...
  //
  // Save to PCD to make sure data consistant
  //
  for (Index = 0; Index < sizeof(mTpmInstanceId)/sizeof(mTpmInstanceId[0]); Index++) {
    if (TrEEConfiguration.TpmDevice == mTpmInstanceId[Index].TpmDevice) {
      PcdSize = sizeof(mTpmInstanceId[Index].TpmInstanceGuid);
      PcdSetPtr (PcdTpmInstanceGuid, &PcdSize, &mTpmInstanceId[Index].TpmInstanceGuid);
      break;
    }
  }
  ...
  )
{
  ...
  if ((Action != EFI_BROWSER_ACTION_CHANGED) || (QuestionId != KEY_TPM_DEVICE)) {
