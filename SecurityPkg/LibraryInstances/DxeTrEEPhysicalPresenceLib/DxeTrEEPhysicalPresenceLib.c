  ...
  )
{
  ...
  TPMS_AUTH_SESSION_COMMAND LocalAuthSession;
  ...
  if (PlatformAuth == NULL) {
    ...
    LocalAuthSession.sessionHandle = TPM_RH_PW;
    LocalAuthSession.auth.t.size = PlatformAuth->t.size;
    CopyMem (LocalAuthSession.auth.t.buffer, PlatformAuth->t.buffer, PlatformAuth->t.size);
  }
...
Done:
  ZeroMem (&LocalAuthSession.auth, sizeof(LocalAuthSession.auth));
  return Status;
}
...
{
  EFI_STATUS                        Status = EFI_SUCCESS; //EIP132975 
  EFI_INPUT_KEY                     Key;
  UINT16                            InputKey;
      
  InputKey = 0; 
  do {
    //Status = gBS->CheckEvent (gST->ConIn->WaitForKey); //EIP132975 (-)
    if (!EFI_ERROR (Status)) {
      Status = gST->ConIn->ReadKeyStroke (gST->ConIn, &Key);
      if ((Key.ScanCode == SCAN_ESC) || (Key.ScanCode == SCAN_DOWN)) {
        InputKey = Key.ScanCode;
      }
      if (((Key.ScanCode == SCAN_F10) || (Key.ScanCode == SCAN_UP)) && !CautionKey) {
        InputKey = Key.ScanCode;
      }
      if (((Key.ScanCode == SCAN_F12) || (Key.ScanCode == SCAN_UP)) && CautionKey) {
        InputKey = Key.ScanCode;
      }
    }      
    Status = gBS->CheckEvent (gST->ConIn->WaitForKey); //EIP132975 
  ...
  if ((InputKey != SCAN_ESC) && (InputKey != SCAN_DOWN)) {
    return TRUE;
  }
  ...
  )
{
  ...
  if (!TrEEHaveValidTpmRequest(TcgPpData, &RequestConfirmed)) {
    //
  ...
  if (RequestConfirmed) {
    TcgPpData->PPResponse = TrEEExecutePhysicalPresence (PlatformAuth, TcgPpData->PPRequest, &TcgPpData->Flags);
  }

  //
  // Clear request
  //
  if ((TcgPpData->Flags & TREE_FLAG_RESET_TRACK) == 0) {
    TcgPpData->LastPPRequest = TcgPpData->PPRequest;
    TcgPpData->PPRequest = TREE_PHYSICAL_PRESENCE_NO_ACTION;    
  }
  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "[TPM2] Flags=%x, PPRequest=%x (LastPPRequest=%x)\n", TcgPpData.Flags, TcgPpData.PPRequest, TcgPpData.LastPPRequest));

  //
  // Execute pending TPM request.
  //  
  TrEEExecutePendingTpmRequest (PlatformAuth, &TcgPpData);
  DEBUG ((EFI_D_ERROR, "[TPM2] PPResponse = %x (LastPPRequest=%x, Flags=%x)\n", TcgPpData.PPResponse, TcgPpData.LastPPRequest, TcgPpData.Flags));
