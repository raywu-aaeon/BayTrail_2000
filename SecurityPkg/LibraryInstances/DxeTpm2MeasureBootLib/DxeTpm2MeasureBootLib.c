  ...
  )
{
  ...
  DEBUG ((EFI_D_ERROR, "DxeImageVerification: MeasureVariable (Pcr - %x, EventType - %x, ", (UINTN)7, (UINTN)EV_EFI_VARIABLE_AUTHORITY));
...
#define AMI_VALID_BOOT_IMAGE_CERT_TBL_GUID \
    ...
    SecureDBBuffer = AllocatePool(VarSize);
  ...
  )
{
  ...
  static BOOLEAN                    bCertMeasure = FALSE;
...
//  EFI_HANDLE                          TempHandle;
  ...
  static    BOOLEAN                   bBootFlag = FALSE;  //  For Reduce the Post Time  (EIP128755+)
//  EFI_FIRMWARE_VOLUME_BLOCK_PROTOCOL  *FvbProtocol;
//  EFI_PHYSICAL_ADDRESS                FvAddress;
//  UINT32                              Index;

  // For Reduce the Post Time (EIP128755+)+>   
  if(!BootPolicy && !bBootFlag)
  {
      return EFI_SUCCESS;
  }
  ...
  if (EFI_ERROR (Status) || !ProtocolCapability.TrEEPresentFlag) {
    ...
    //
    DEBUG ((EFI_D_ERROR, "DxeTpm2MeasureBootHandler - TrEEPresentFlag - %x\n", ProtocolCapability.TrEEPresentFlag));
    return EFI_SUCCESS;
  }
