	...
	IN VOID *SystemTable)
{
	...
	DbgData->m_ppPeiSVC = ((EFI_SYSTEM_TABLE*)(SystemTable));
	...
	OUT	UINT32	*USBDebugPortStartAddr)
{
	...
	if(USBDebugPortStartAddr)
		{
			DbgPrtCapReg = FindCapPtr(PciRootBrdgIoProtocol, addr,DBG_PRT_CAP_ID);
...
//
// Input:		AMI_DEBUGPORT_INFORMATION_HOB *This,
//				DEBUG_PORT_INFO *pDeviceInfo
//
// Output:		EFI_STATUS
//--------------------------------------------------------------------
//<AMI_PHDR_END>
