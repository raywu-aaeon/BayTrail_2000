...
ifneq ("$(PeiDebugger_SUPPORT)", "1")
  ifneq ("$(GENERIC_USB_CABLE_SUPPORT)", "1")
    $(error  Dxe Only Debugging is not supported yet, try Generic USB Debugger.)
  endif
endif

# Make sure that Serial Debugger and Usb Debug transport tokens are not enabled togather
ifneq ("$(USB_DEBUG_TRANSPORT)", "1")
  ifneq ("$(SERIAL_DEBUGGER_SUPPORT)", "1")
	ifneq ("$(GENERIC_USB_CABLE_SUPPORT)", "1")  
	    $(error  To enable Serial debugging switch on SERIAL_DEBUGGER_SUPPORT SDL token)
  	endif
  endif
endif
