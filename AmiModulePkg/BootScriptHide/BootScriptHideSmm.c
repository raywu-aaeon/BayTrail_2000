...
/**
 * @file
 * Source file for the SMM driver. Contains the code that is signaled 
 * when it is time to save/restore the boot script tables
 */
#include <AmiDxeLib.h>
#include <Token.h>
#include <AmiAcpiS3.h>
...
/**
 * Function that verifies that the passed address is not contained in the region of 
 * memory occupied by the SMM code. 
 * 
 * @param Buffer Address to check against the SMM memory region
 * @param BufferSize Size of the buffer to check against the Smm Memory Region
 * 
 * @return BOOLEAN
 * @retval True The address is part of the SMRAM region
 * @retval False The address is not part of the SMRAM region
 */
...
/**
 * Using the passed BootScriptTablePointer, go through the entries and determine the total size of the Boot Script table.
 * 
 * @param BootScriptTablePtr Pointer to the boot script tables
 * 
 * @return UINTN The total size of the boot script tables
 */
...
/**
 * SMM Dispatch handler to save the Boot Scripts from regular system memory into SMRAM.
 * 
 * @param DispatchHandle Dispatch handle for this handler
 * @param Context  Context for this dispatch 
 * @param ComBuffer Pointer to the buffer containing communication data
 * @param CommBufferSize Pointer to the size of this communication buffer
 * 
 * @return EFI_STATUS
 * @retval EFI_SUCCESS Boot Scripts successfully saved
 * @retval EFI_NOT_FOUND The boot scripts were not found
 */
EFI_STATUS EFIAPI SaveBootScriptHandle(
  ...
  IN OUT UINTN   *CommBufferSize  OPTIONAL
    ...
    Status = pRS->GetVariable(
      AcpiGlobalVariable, &EfiAcpiVariableGuid,
...
/**
 * SMM Dispatch handler to restore the Boot Scripts from SMRAM to the regular system memory
 * 
 * @param DispatchHandle Dispatch handle for this handler
 * @param Context  Context for this dispatch 
 * @param ComBuffer Pointer to the buffer containing communication data
 ...
 * 
 * @return EFI_STATUS
 * @retval EFI_SUCCESS Boot Scripts sucessfully restored
 * @retval EFI_NOT_FOUND The boot scripts were not saved, and could not be restored
 */
...
/**
 * SMM driver entry point.  This entry point will register SWSMI dispatch handlers to deal with the save and restore SWSMIs.
 * 
 * @param ImageHandle The handle that corresponds this this loaded SMM driver
 * @param SystemTable Pointer to the EFI System Table
 * 
 * @return EFI_STATUS
 * @return EFI_SUCCESS The driver was successfully loaded
 */
