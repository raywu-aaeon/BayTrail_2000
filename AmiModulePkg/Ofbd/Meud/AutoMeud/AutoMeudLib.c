...
//**********************************************************************
/** @file AutoMeudLib.c
    If system updated BIOS with warm reset, PeiRamBoot module
    ...
    switch( SectionHeader->Type )
    {
        case EFI_SECTION_COMPRESSION:
            ...
            if( UncompressedLength > 0 )
            {
                ...
                {
                    Status = pBS->LocateProtocol( &gEfiDecompressProtocolGuid,
                                                  NULL,
                                                  &Decompress );
                    if( EFI_ERROR(Status) )
                    {
...
)
{
    ...
    if (FvHeader->Signature != EFI_FVH_SIGNATURE) return NULL;
    if (!guidcmp((UINT8*)StartAddress + FvHeader->ExtHeaderOffset, \
    ...
    if (FvHeader->Signature != EFI_FVH_SIGNATURE) return NULL;
    if (!guidcmp((UINT8*)StartAddress + FLASH_SIZE + FvHeader->ExtHeaderOffset, \
...
/**
    ...
    @retval VOID
**/
VOID AutoMeudAfterReflashHook (VOID)
{
...
/**
    ...
    @retval VOID
**/
VOID AutoMeudBeforeReflashHook (VOID)
{
...
//-#endif    
	...
	Status = pRS->GetVariable (L"MbpMeFwVersion", \
                        &gMeFwUpdVariableGuid, NULL, &Length, &MbpMeFwVer);
    ...
    if (RecoveryBuffer == GetFvMeFwRegionAddress(RecoveryBuffer)) {
        ...
        if (gAmiPostMgr != NULL) 
            gAmiPostMgr->DisplayInfoBox(L"ME FW update", L"ME FW update completed, Press any key to reset the system", 5, &event);
...
//-        if ((BlocksToUpdate[i].Update == TRUE) || \
//-            (BlocksToUpdate[i].BlockAddress != (UINT8*)FV_DATA_BASE)) continue;
//-           
