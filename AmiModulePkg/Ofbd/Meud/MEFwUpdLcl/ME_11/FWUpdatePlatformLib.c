...
)
{
    ...
    ResetData = PchReset->GetResetData (&gPchGlobalResetGuid, &DataSize);
    PchReset->Reset (PchReset, EfiResetPlatformSpecific, DataSize, ResetData);
    return EFI_SUCCESS;
}
  ...
  )
{
    if (gAmiPostMgr != NULL) {
        ...
        if (PercentWritten == 100) gAmiPostMgr = NULL;                              
    }                                 
}
...
)
{
    ...
    // PDT Data Update.
    if(FirmwareType == PDT_DATA) {
        Status = HeciPdt (FileBuffer, (UINT32)FileLength);
        if (This->AmiPostMgr != NULL) {
            EFI_EVENT   Event;
            if (!EFI_ERROR(Status)) This->AmiPostMgr->DisplayInfoBox(\
                L"PDT Data update", L"Flash New PDT Data Completed.", 1, &Event);
            else This->AmiPostMgr->DisplayInfoBox(L"PDT Data update",\
                                    L"Flash New PDT Data Failed.", 1, &Event);   
        }                                
        return Status;                        
    }
    ...
    {
        UINT32             j, ComparePartID = 0, PartId = 0;
        BOOLEAN            found = FALSE;
        ...
        if (FWU_ERROR_SUCCESS != (FWUpdateStatus = GetExtendedIpuPartitionAttributes( \
                        &FwuGetIpuAttrbMsgInfo, FWU_IPU_UPDATE_OPERATION))) {
       ...
       //Loop through expected partitions from FW to find partition requested
        ...
        if(!found) return FWU_PID_NOT_EXPECTED;
