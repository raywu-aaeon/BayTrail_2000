...
//--------------------------------
    ...
    LdGotoData->LdFormTitleStringId=HiiSetString(gSioHiiHandle, 0, s, NULL );
...
VOID CreateLdGotoItems(SIO_IFR_INFO *SioIfrInfo)
{
    UINTN 			i;
    EFI_STATUS 		Status;
...
//----------------------------------------------
    ...
    //------------------------------
    	...
    	GotoId=SIO_GOTO_ID_BASE+(UINT16)i;
...
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
EFI_VARSTORE_ID GetVarStoreID(UINTN SioIdx, UINTN LdIdx, BOOLEAN NoneVolatile){
	SIO_VAR_ID	varid;
//-----------------------	
	varid.IdField.Always1=1;
	varid.IdField.IsNoneVolatile=NoneVolatile;
	varid.IdField.SioIndex=(UINT8)SioIdx;
	varid.IdField.LdIndex=(UINT8)LdIdx;
	return (EFI_VARSTORE_ID)(varid.VAR_ID);
}


VOID UpdateCurrentResourcesStrId(LD_SETUP_GOTO_DATA *GotoData){
	EFI_STRING 	s, configs;
	CHAR16		strbuf[61];
...
//EFI_DEADLOOP();    
    ...
    vsid=GetVarStoreID(gotodata->SioIdx,gotodata->LdIdx, TRUE),
	//SIO_TRACE((TRACE_SIO,"SioSetup: SIO LD varstoreid=0x%X.\n.",vsid));
    ...
    AmiHiiCreateSuppresGrayIdVal(gLdConfigForm.StartOpCodeHandle, SIO_LD_ENABLE_QID, 0,TRUE);	
...
//====================================================
EFI_STRING GenerateResourceString(EFI_ASL_DepFn *Resources){
	CHAR16			s[80];
...
//====================================================
VOID UpdateConfigurationStrings(LD_SETUP_GOTO_DATA *GotoData){
...
EFI_STATUS InitSioIfrData (SIO_IFR_INFO *SioIfrInfo)
{
    EFI_STATUS  				Status;
...
//EFI_DEADLOOP();
    ...
    //---------------------------	
    	...
    	//Presence of Our Propriatary Interface constitutes following type casting...
...
VOID InitSioIfr(IN EFI_EVENT Event, IN VOID *Context)
{
	EFI_STATUS			Status;
//------------------------------------
...
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
VOID SortData(SIO_IFR_INFO* IfrData){
