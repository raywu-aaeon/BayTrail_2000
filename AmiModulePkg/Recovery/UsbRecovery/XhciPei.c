...
)
{
    ...
    EFI_PHYSICAL_ADDRESS    TempPtr;
    UINTN                   AlignmentMask;
    UINTN                   Base = (UINTN)Usb3Hc->ScratchpadBufArray;
    ...
    Status = (**PeiServices).AllocatePages(PeiServices, 
                                EfiBootServicesData,
                                Usb3Hc->OpRegs->PageSize * (NumBufs + 1),
                                &TempPtr);
    ...
    (**PeiServices).SetMem((VOID*)TempPtr, Usb3Hc->OpRegs->PageSize * 4096 * (NumBufs + 1), 0);

    AlignmentMask = (Usb3Hc->OpRegs->PageSize << 12) - 1;

    TempPtr = ((UINTN)TempPtr +AlignmentMask) & ~AlignmentMask;

    for (Count = 0; Count < NumBufs; Count++) {    
        // Update *ScratchBufArrayBase
        *(UINTN*)Base = (UINTN)TempPtr + (Usb3Hc->OpRegs->PageSize << 12) * Count;
        Base += 8;
    }

    // Update scratchpad pointer
    *(UINTN*)Usb3Hc->DcbaaPtr = (UINTN)Usb3Hc->ScratchpadBufArray;
...
)
{
    ...
    PEI_ASSERT(PeiServices, Usb3Hc->OpRegs->UsbCmd.Field.HcRst == 0);
    if (Usb3Hc->OpRegs->UsbCmd.Field.HcRst) return EFI_DEVICE_ERROR;  // Controller can not be reset
    ...
    PEI_ASSERT(PeiServices, Usb3Hc->OpRegs->UsbSts.Field.HcHalted == 0);
...
//    XHCI_FIXED_DELAY_MS(Usb3Hc, 1000);
    ...
    XHCI_FIXED_DELAY_MS(Usb3Hc, 2);  // 2 msec
...
)
{
    ...
    *Doorbell = Dci;
    ...
    PEI_ASSERT(PeiServices, EpCtx->EpState == XHCI_EP_STATE_RUNNING);
    ...
    OUT UINT32                     *TransferResult )
{
    ...
    *(UINT16*)&((XHCI_SETUP_XFR_TRB*)Trb)->bmRequestType = Rq;
    ...
    XhciRingDoorbell(PeiServices, Usb3Hc, SlotId, 1);
    ...
    OUT UINT32                     *TransferResult )
{
    ...
    *TransferResult = EFI_USB_NOERROR;
    ...
    for (TransferredSize = 0; TransferredSize < *DataLength;) {
        ...
        XhciRingDoorbell(PeiServices, Usb3Hc, SlotId, Dci);
    ...
    OUT EFI_USB_PORT_STATUS        *PortStatus )
{
    ...
    *(UINT32*)PortStatus = 0;
    ...
    if ((PortSC->Field.Prc != 0) ||(PortSC->Field.Wrc != 0))  {
    ...
    IN EFI_USB_PORT_FEATURE        PortFeature )
{
    ...
    switch (PortFeature) {
        ...
        case EfiUsbPortResetChange:
            PortSC->AllBits = (PortSC->AllBits & XHCI_PCS_PP) | XHCI_PCS_PRC |XHCI_PCS_WRC;
            break;
...
//
// Output:
//      EFI_SUCCESS     - if the Xhci host controller is reset successfully.
//      EFI_UNSUPPORTED - if unknow attribute is supplied
//
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
...
{    
    UINT32                   i;
    USB3_CONTROLLER          *Usb3Hc = PEI_RECOVERY_USB_XHCI_DEV_FROM_THIS(This);
    volatile XHCI_HC_OP_REGS *OpRegs = Usb3Hc->OpRegs;	
    XHCI_HC_RT_REGS	  	     *RtRegs = Usb3Hc->RtRegs;
    XHCI_INTERRUPTER_REGS    *Interrupter = RtRegs->IntRegs;
    
    switch (Attributes) {
        case EFI_USB_HC_RESET_GLOBAL:
        
            // Disable interrupt				
            OpRegs->UsbCmd.Field.Inte = 0;
            Interrupter->IMan &= ~BIT1;
            
            // Clear the Run/Stop bit
            OpRegs->UsbCmd.Field.RunStop = 0;
            
            // The xHC should halt within 16 ms. Section 5.4.1.1
            for (i = 0; i < 32; i++) {
                XHCI_FIXED_DELAY_MS(Usb3Hc, 1);    // 1ms delay 				
                if (OpRegs->UsbSts.Field.HcHalted) {
                    break;
                }
            }
            PEI_ASSERT(PeiServices, OpRegs->UsbSts.Field.HcHalted == 1);
        
            // Reset controller
            OpRegs->UsbCmd.Field.HcRst = 1;
            for (i = 0; i < 400; i++) {
                XHCI_FIXED_DELAY_MS(Usb3Hc, 1);    // 1ms delay
                if (OpRegs->UsbCmd.Field.HcRst == 0) {
                    break;
                }
            }
            PEI_ASSERT(PeiServices, OpRegs->UsbCmd.Field.HcRst == 0);            
