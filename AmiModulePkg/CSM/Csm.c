...
)
{
    ...
    if (gSetTxtMode == 1 && gAmiCsmVideoPolicy != NULL)
    {
...
)
{
    ...
    TRACE((-1, "HI PMM memory allocated at 0x%x, size 0x%x: %r\n", HiPmmMemory, HiPmmMemorySizeInBytes, Status));
    ASSERT_EFI_ERROR(Status);
...
/* PROJECT specific data is not found in AptioV, uncomment when it is available
    ...
    for(i = 0; i < 8; i++) {
     ...
     Status = GetPlatformInfoEx(&gAmiCsmExtensionsProtocol,
...
)
{
    ...
    *HddInfo  = (struct _HDD_INFO*)gCoreBiosInfo->Thunk->Csm16BootTable.HddInfo;
    *BbsCount = gCoreBiosInfo->BbsEntriesNo;
    *BbsTable = (struct _BBS_TABLE*)gCoreBiosInfo->BbsTable;
...
//    RemoveDecodingForShadow((UINT32)gNextRomAddress);
    ...
    Status = gCoreBiosInfo->iBiosPlatform->PrepareToBoot(
        gCoreBiosInfo->iBiosPlatform,
        BootOption,
        ...
        &gCoreBiosInfo->Thunk->Csm16BootTable
					...
					//(EIP95347+)
//    if (!gIsBootPrepared) { // EIP52733
        Status = PrepareToBoot(This, &BbsCount, (struct _BBS_TABLE**)&BbsTable);
//    } // EIP52733
