...
AMI_OPROM_POLICY_PROTOCOL *gCsmPolicy = NULL;
AMI_CSM_VIDEO_POLICY_PROTOCOL *gAmiCsmVideoPolicy = NULL;
...
)
{
	...
	Status = GetOpromVideoSwitchingMode(PciIo, gSetTxtMode, &SetTxtMode);
    if (EFI_ERROR(Status)) {
        SetTxtMode = (UINTN)gSetTxtMode;
    }
    if (!IsVga)
    {
        // locate and use VideoPolicy GetMode/SetMode protocol
        if (gAmiCsmVideoPolicy == NULL)
        {
            Status = pBS->LocateProtocol(&gAmiCsmVideoPolicyProtocolGuid, NULL, &gAmiCsmVideoPolicy);
            ASSERT_EFI_ERROR(Status);
        }

        if (SetTxtMode != 2) {
            BOOLEAN IsText;
            gAmiCsmVideoPolicy->GetMode(gAmiCsmVideoPolicy, &IsText);
            if (!IsText) {
                Status = gAmiCsmVideoPolicy->SetMode(gAmiCsmVideoPolicy, TRUE, FALSE); // switch to text mode
                // clear screen
                if (!EFI_ERROR(Status)) {
                    RegSet.X.AX = 3;
...
TRACE((-1, "InstallRom...Run-time ROM Size = %x Bytes\n", RtRomSize));
    ...
    *NewRomSize = RtRomSize;
    ...
    if (SetTxtMode == 0)
    {
        gAmiCsmVideoPolicy->SetMode(gAmiCsmVideoPolicy, FALSE, FALSE); // switch to graphics mode
    }
...
//    CSM_PLATFORM_PROTOCOL *CsmPlatformProtocol;
	...
	Status = PreProcessOpRom(PciIo, RomImage);
...
)
{
    ...
    if (gCsmPolicy == NULL) {
        pBS->LocateProtocol(&gAmiOpromPolicyProtocolGuid, NULL, &gCsmPolicy);
    }
