...
EFI_DEVICE_PATH_PROTOCOL* AmiImageTypeLibFindDevicePathNode(
	EFI_DEVICE_PATH_PROTOCOL *DevicePath, UINT8 Type, UINT8 SubType
){
    if (DevicePath == NULL) return NULL;

    for( ; !IsDevicePathEnd(DevicePath); DevicePath=NextDevicePathNode(DevicePath) )
        if (DevicePath->Type == Type && DevicePath->SubType == SubType) return DevicePath;
    return NULL;
}
