...
//--------------------
    ...
    do{
        ...
        //Here lmpl holds smallest value of the link check if accumulated MPL exceeds MPL of this link...
...
//#endif
    ...
    //for(i=0, ltok=FALSE; i<=gPciSetupData->LnkTrRetry; i++){
        ...
        if(sta.LnkTraining!=0 && i>=RetryCount){
            ...
            ASSERT_EFI_ERROR(Status);
            goto rEXIT;
        } else PCI_TRACE((TRACE_PCI,"tried %d, ",i+1));
...
//--------------------
    ...
    do{
        ...
        up=dn->ParentBrg;
    } while (PcieCheckPcieCompatible(dn->ParentBrg) && dn->ParentBrg->Type!=tPciRootBrg);//(EIP49743)
...
//-------------------------
	...
	aspmstore=aspm;
	if( (DnPort->AmiSdlPciDevData!=NULL && DnPort->AmiSdlPciDevData->InitRoutine!=0)||
		(UpPort->AmiSdlPciDevData!=NULL && UpPort->AmiSdlPciDevData->InitRoutine!=0)  )
	{
	
	//	
		if( DnPort->AmiSdlPciDevData!=NULL && DnPort->AmiSdlPciDevData->InitRoutine!=0 ){
			Status=LaunchInitRoutine(DnPort, isPcieSetAspm, itDevice, DnPort, UpPort, &aspm, NULL);
		} else {
			Status=LaunchInitRoutine(UpPort, isPcieSetAspm, itDevice, DnPort, UpPort, &aspm, NULL);
		}
		
		if(EFI_ERROR(Status)){
			if(Status==EFI_UNSUPPORTED){
				aspm=aspmstore;
				PCI_TRACE((TRACE_PCI,"\n  A_V: Don't Overwrite ASPM = 0x%X: %r\n", aspm, Status));
		    ...
		    PCI_TRACE((TRACE_PCI,"\n  A_V: Overwrites ASPM=0x%X; Old ASPM=0x%X: %r\n", aspm, aspmstore, Status));
		}  
	} else {
		//this is situation when NO AptioV native routines specified and ...
		//we would check for  Aptio4.x Compatible porting...
		Status=LaunchPortingRoutine(DnPort, isPcieSetAspm, DnPort, UpPort, &aspm, NULL);
		if(EFI_ERROR(Status)){
			if(Status==EFI_UNSUPPORTED){
				aspm=aspmstore;
				PCI_TRACE((TRACE_PCI,"\n  A_4: Don't Overwrite ASPM = 0x%X: %r\n", aspm, Status));
				//just in case if function returned UNSUPPORTED modified ASPM settings - Restore it...
				Status=EFI_SUCCESS;
			} else {
				ASSERT_EFI_ERROR(Status);
			...
			//this is SUCCESS status branch..
		    PCI_TRACE((TRACE_PCI,"\n  A_4: Overwrites ASPM=0x%X; Old ASPM=0x%X: %r\n", aspm, aspmstore, Status));
		}  
	}
...
//-------------------------
	...
	if(MaxReadRequestOption!=PCI_SETUP_AUTO_VALUE) devc.MaxReadReq=MaxReadRequestOption;
...
//    else devc.MaxReadReq=PCIE_MAXPL_512B;
	...
	devc.RelaxOrder=RelaxedOrderingOption;
	devc.NoSnoop=NoSnoopOption;
...
//#if SRIOV_SUPPORT
        if (extcap.ExtCapId==PCIE_CAP_ID_SRIOV && SriovSupport ){
        	Status=SriovAllocateInitSriovData(Device, addr.Addr.ExtendedRegister, MaxBusFound);
            ASSERT_EFI_ERROR(Status);
        }
//#endif
