...
AMI_DEBUG_SERVICE_PROTOCOL *GetAmiDebugServicesProtocolInstance (VOID)
{
    ...
    Status = gSmst->SmmLocateProtocol(&gAmiSmmDebugServiceProtocolGuid, NULL, &AmiDebugServiceProtocol);
  ...
  )
{
    ...
    if(AmiDebugServiceProtocol == NULL || (ErrorLevel & GetDebugPrintErrorLevel ()) == 0) return;
