...
BOOLEAN FtFvBeforePublishing (IN CONST EFI_PEI_SERVICES **PeiServices, IN OUT ROM_AREA *Area, IN UINT32 FvType)
{
return TRUE;
}

BOOLEAN teste1 (IN CONST EFI_PEI_SERVICES **PeiServices, IN OUT ROM_AREA *Area, IN UINT32 FvType)
{
return TRUE;
}
/**
    ...
    IN VOID *NullPpi)
{
    ...
    if(EFI_ERROR(Status))
    {
        DEBUG((-1, "WARNING: Unable to determine BootMode\n"));
    ...
    for(Area = AmiGetFirstRomArea(); Area != NULL; Area = AmiGetNextRomArea(Area))
    {
        ...
        {
            DEBUG((-1, "ERROR: Corrupt FirmwareVolume at address 0x%p\n", Current.Address));
...
EFI_STATUS ReportFV2Dxe(IN VOID* RecoveryCapsule OPTIONAL, IN CONST EFI_PEI_SERVICES **PeiServices)
{
    ...
    if(RecoveryCapsule != NULL)
    {
        ...
        if(EFI_ERROR(Status))
        {
            DEBUG((-1, "ERROR: ROM Layout is not found in the Recovery Image\n"));
            DEBUG((-1, "       Recovery will fail and boot will be attempted using current ROM image\n"));
            return Status;
        }
...
//                RomLayoutHobEntry->Area.Address = (EFI_PHYSICAL_ADDRESS)((UINTN)RecoveryCapsule + Next->Offset);
            }
        }
    }
