...
/**
    Installs Ata Pass Thru APIs provides services allowing
...
/**
    Stops the Ata Pass Thru Support security interface.
...
)
{
    ...
    //
    // Returns the Device Interface of Correspoding Port/PM
    // 
...
CountAhciDrives (
    ...
    //
    pBS->HandleProtocol(HandleBuffer, &gAmiAhciBusProtocolGuid, (VOID **)&AhciBusInterface);
...
CountIdeDrives (
    ...
    // With the Controller handle, find the total devices present and their Address
    //
    pBS->HandleProtocol(HandleBuffer,&gAmiIdeBusInitProtocolGuid,(VOID **)&IdeBusInitProtocol);    
...
GetAhciAsbStatus (
  ...
  IN    OUT EFI_ATA_STATUS_BLOCK    *Asb
    ...
    UINT64    AhciBaseAddr = SataDevInterface->AhciBusInterface->AhciBaseAddress; //(CSP20150309)
    ...
    if(ModeFlag) {
        switch(Packet->Protocol){
            ...
            case  EFI_ATA_PASS_THRU_PROTOCOL_ATA_NON_DATA:
                  Status = SataDevInterface->AhciBusInterface->ExecuteNonDataCommand (  SataDevInterface, 
                                                                                        CommandStructure 
                                                                                     );
            ...
            case EFI_ATA_PASS_THRU_PROTOCOL_PIO_DATA_IN:
                  Status = SataDevInterface->AhciBusInterface->ExecutePioDataCommand (  SataDevInterface, 
                                                                                        &CommandStructure, 
                                                                                        FALSE
                                                                                     );
            ...
            case EFI_ATA_PASS_THRU_PROTOCOL_PIO_DATA_OUT:
                  Status = SataDevInterface->AhciBusInterface->SataPioDataOut(  SataDevInterface, 
