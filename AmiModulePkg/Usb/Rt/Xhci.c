...
UINT8   XHCI_GetRootHubStatus (HC_STRUC*,UINT8);
...
UINT64      XHCI_Mmio64Read(USB3_HOST_CONTROLLER*, UINTN);
VOID        XHCI_Mmio64Write(USB3_HOST_CONTROLLER*, UINTN, UINT64);
...
VOID		XhciRingDoorbell(USB3_HOST_CONTROLLER*, UINT8, UINT8);
EFI_STATUS  XhciExtCapParser(USB3_HOST_CONTROLLER*);

DEV_INFO*   XHCI_GetDevInfo(UINTN);
UINT8*		XHCI_GetDeviceContext(USB3_HOST_CONTROLLER*, UINT8);
UINT8*		XHCI_GetContextEntry(USB3_HOST_CONTROLLER*, VOID*, UINT8);
...
//    ASSERT(Usb3Hc->OpRegs->UsbSts.Field.Cnr == 0);
    ...
    for (i = 0; i < 8000; i++) {
        if (Usb3Hc->OpRegs->UsbCmd.Field.HcRst == 0) break;
        FixedDelay(100);    // 100 us delay
    }
    ASSERT(Usb3Hc->OpRegs->UsbCmd.Field.HcRst == 0);
    if (Usb3Hc->OpRegs->UsbCmd.Field.HcRst) return USB_ERROR;  // Controller can not be reset
    ...
    XHCI_Mmio64Write(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->DcbAap, (UINT64)(UINTN)Usb3Hc->DcbaaPtr);

	// Check if xHC support 64bit access capability
	if (Usb3Hc->Access64) {
		if(XHCI_Mmio64Read(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->DcbAap) != (UINT64)(UINTN)Usb3Hc->DcbaaPtr) {
			Usb3Hc->Access64 = 0;
			XHCI_Mmio64Write(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->DcbAap, (UINT64)(UINTN)Usb3Hc->DcbaaPtr);
		}
	}
    ...
    XHCI_Mmio64Write(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->Crcr,
    ...
    // ER dequeue pointer
    XHCI_Mmio64Write(Usb3Hc, (UINTN)&Interrupter->Erdp, (UINT64)(UINTN)Usb3Hc->EvtRing.QueuePtr);
    // Seg Table location
    XHCI_Mmio64Write(Usb3Hc, (UINTN)&Interrupter->Erstba, (UINT64)(UINTN)Erst0Entry);
...
#else
    //
    // Force Periodic SMI for XHCI since BayTrail doesn't support bit31 of SMI_EN
    //
    USBSB_InstallXhciHwSmiHandler();
#endif
//EIP128872 <<
...
)
{
	...
	if (XHCI_Mmio64Read(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->Crcr) & CRCR_COMMAND_RUNNING) {
		// Stop the command ring
		XHCI_Mmio64Write(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->Crcr, CRCR_COMMAND_STOP);
    ...
    *Doorbell = 0;  // HC doorbell is #0
    ...
    if (Status == USB_ERROR) {
		...
		if (CompletionCode == XHCI_TRB_EXECUTION_TIMEOUT_ERROR) {
			XHCI_Mmio64Write(Usb3Hc, (UINTN)&Usb3Hc->OpRegs->Crcr, CRCR_COMMAND_ABORT);
...
*/
    ...
    Status = XHCI_ExecuteCommand(HcStruc, XhciTResetEndpointCmd, &EpInfo);
//EIP128872 >>
#if (XHCI_TIMER_EVENT_SMI == 0) && (USB_RUNTIME_DRIVER_IN_SMM == 1)
    if ( (Status != USB_SUCCESS) && (0 == Usb3Hc->DcbaaPtr->DevCntxtAddr[SlotId-1]) ){
      USB_DEBUG(3, "XHCI_ClearStalledEp for SlotID %d that has been disconnected\n", SlotId);
      return EFI_SUCCESS;
    }
#endif
//EIP128872 <<
...
)
{
    ...
    Trb->XferLength = DevInfo->wIntMaxPkt;				//(EIP54782)
	Trb->Isp = 1;					//(EIP51478+)
	Trb->Ioc = 1;
...
)
{
	...
	if (XhciBaseAddress != (UINTN)Usb3Hc->CapRegs) {
        Usb3Hc->CapRegs = (XHCI_HC_CAP_REGS*)XhciBaseAddress;
...
//    Usb3Hc->OpRegs->UsbSts.AllBits = XHCI_STS_EVT_INTERRUPT;    // Clear event interrupt
    ...
    if (Usb3Hc->EvtRing.QueuePtr->CycleBit != Usb3Hc->EvtRing.CycleBit) return USB_SUCCESS;
...
)
{
    ...
    if (PortSC->Field.Ccs != 0) {
        ...
        // set the connect change flag if the Port Reset Change or Warm port 
        // reset change are set.
        if (gUsbData->bIgnoreConnectStsChng == TRUE) {

            if (((PortSC->Field.Prc == 1) || (PortSC->Field.Wrc == 1)) && 
					(PortSC->Field.Csc == 0)) {
...
)
{
    ...
    if (Request == USB_RQ_SET_ADDRESS) return Length;

    ASSERT(DevInfo != NULL);

	if(DevInfo->bFlag & DEV_INFO_DEV_DISCONNECTING) return 0;	//(EIP60460+)
    ...
    *(UINT16*)&((XHCI_SETUP_XFR_TRB*)Trb)->bmRequestType = Request;
    ...
    XhciRingDoorbell(Usb3Hc, SlotId, 1);
...
)
{
    ...
    for (TransferredSize = 0; TransferredSize < Length;) {
        ...
        // Ring the door bell and see Event Ring update
        XhciRingDoorbell(Usb3Hc, SlotId, Dci);
...
)
{
	...
	USB_MemFree(DevInfo->fpPollTDPtr, GET_MEM_BLK_COUNT(DevInfo->wIntMaxPkt));
...
)
{
    ...
    Dci = (DevInfo->bIntEndpoint & 0xF) * 2;
    if (DevInfo->bIntEndpoint & BIT7) Dci++;
    DevInfo->fpPollTDPtr = USB_MemAlloc(GET_MEM_BLK_COUNT(DevInfo->wIntMaxPkt)); 	//(EIP54782)
    XfrRing = XHCI_GetXfrRing(Usb3Hc, SlotId, Dci - 1);

    Trb = XHCI_AdvanceEnqueuePtr(XfrRing);
    Trb->TrbType = XhciTNormal;
    ((XHCI_NORMAL_XFR_TRB*)Trb)->DataBuffer = (UINT64)(UINTN)DevInfo->fpPollTDPtr;
    ((XHCI_NORMAL_XFR_TRB*)Trb)->XferLength = DevInfo->wIntMaxPkt;		//(EIP54782)
...
)
{
    ...
    ASSERT(Status == USB_SUCCESS);
    ASSERT(SlotId != 0);
	if(Status != USB_SUCCESS) return Status;

	Device = (UINT8*)XHCI_GetDeviceContext(Usb3Hc, SlotId);
    MemSet(Device, XHCI_DEVICE_CONTEXT_ENTRIES * Usb3Hc->ContextSize, 0);
    ...
    SlotCtx->RootHubPort = XHCI_GetRootHubPort(DevInfo);

    switch (PortStatus & USB_PORT_STAT_DEV_SPEED_MASK) {
        case USB_PORT_STAT_DEV_HISPEED: Speed = XHCI_DEVSPEED_HIGH; break;
        case USB_PORT_STAT_DEV_LOWSPEED: Speed = XHCI_DEVSPEED_LOW; break;
        case USB_PORT_STAT_DEV_FULLSPEED: Speed = XHCI_DEVSPEED_FULL;
    }
    if (PortStatus & USB_PORT_STAT_DEV_SUPERSPEED) {
    ...
    Ep0Ctx = (XHCI_EP_CONTEXT*)XHCI_GetContextEntry(Usb3Hc, (UINT8*)Usb3Hc->InputContext, 2);
    ...
    if (ParentHub != NULL) {
        ParentHubSlotCtx = (XHCI_SLOT_CONTEXT*)XHCI_GetContextEntry(Usb3Hc, (UINT8*)ParentHub->DevMiscInfo, 0);
		if (ParentHubSlotCtx->Speed == XHCI_DEVSPEED_SUPER) {
	        HubPortNumber = (DevInfo->bHubPortNumber > 15)? 15 : DevInfo->bHubPortNumber;
			SlotCtx->RouteString = ParentHubSlotCtx->RouteString | 
										(HubPortNumber << (ParentHub->HubDepth * 4));	//(EIP51503)
		} else {
	        // Update TT fields in the Slot context for LS/FS device connected to HS hub
	        if (SlotCtx->Speed == XHCI_DEVSPEED_FULL ||
	            SlotCtx->Speed == XHCI_DEVSPEED_LOW) {
	            if(ParentHubSlotCtx->Speed == XHCI_DEVSPEED_HIGH) {
					SlotCtx->TtHubSlotId = XHCI_GetSlotId(Usb3Hc, ParentHub);
	                SlotCtx->TtPortNumber = DevInfo->bHubPortNumber;
    ...
    Status = XHCI_ExecuteCommand(HcStruc, XhciTAddressDeviceCmd, &SlotId);
	if(Status != USB_SUCCESS) {
    ...
    *DeviceData = (UINT8*)Device;

	FixedDelay(2 * 1000);	// 2 ms delay	//(EIP60868+)
    return USB_SUCCESS;
}
