...
)
{
    ...
    AMI_AHCI_BUS_PROTOCOL       *AhciBusInterface = SataDevInterface->AhciBusInterface; 
    UINT8                       Port = SataDevInterface->PortNumber;
    UINT32                      Data32_SERR, Data32_IS, Data32_CI;
    UINT32                      i=1;
...
)
{
    ...
    if (EFI_ERROR(Status)) {
        DEBUG ((DEBUG_ERROR, " AHCI: GeneratePortReset Status : %r\n", Status));
        return EFI_DEVICE_ERROR;
    }

    DEBUG ((DEBUG_INFO, "AHCI : GeneratePortReset Status : %r\n", Status));
    return EFI_SUCCESS;    
    ...
    if (DeviceDetected) {
        ...
        for (i = 0; i < 100; i++){   // Total delay 10msec        
            ...
            if((FISAddress->Ahci_Rfis[2] & HBA_PORTS_TFD_MASK) == \
                       (HBA_PORT_REG32 (AhciBusInterface, Port, HBA_PORTS_TFD) & HBA_PORTS_TFD_MASK)) break;
...
)
{
    ...
    UINT32  Data32;
    UINT32  TimeoutCount;
    UINT32  AhciBaseAddr = (UINT32)(SataDevInterface->AhciBusInterface);
    AMI_AHCI_BUS_PROTOCOL  *AhciBusInterface = SataDevInterface->AhciBusInterface; 
