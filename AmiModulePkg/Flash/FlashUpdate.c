...
)
{
    ...
    for (Retry = 0; Retry < FLASH_RETRIES; Retry++) {
        if (Completed = FlashIsEraseCompleted(BlockAddress, &Error, NULL)) break;
    }        
...
)
{
    ...
    for (Retry = 0; Retry < FLASH_RETRIES; Retry++) {
        if (Completed = FlashIsProgramCompleted ( \
                            Address, Data, Length, &Error, NULL)) break;
    }
