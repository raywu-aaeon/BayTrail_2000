    ...
    IN EFI_USB_PORT_FEATURE        PortFeature )
{
    ...
    case EfiUsbPortSuspend:
        OhciHcReg->HcRhPortStatus[PortNumber].AllBits = SET_PORT_SUSPEND;
        break;

    case EfiUsbPortReset:
        OhciHcReg->HcRhPortStatus[PortNumber].AllBits = SET_PORT_RESET;
        break;
    ...
    case EfiUsbPortEnable:
        OhciHcReg->HcRhPortStatus[PortNumber].AllBits = SET_PORT_ENABLE;
        break;
    ...
    IN EFI_USB_PORT_FEATURE        PortFeature )
{
    ...
    Pstatus = &OhciHcReg->HcRhPortStatus[PortNumber];
    ...
    case EfiUsbPortPower:
        ...
        // was implemented.
    case EfiUsbPortReset:
        // First wait no more that 10ms if the port reset is not complete
        if (Pstatus->Field.PortResetStatusChange == PORT_RESET_NOT_COMPLETE){
            OHCI_FIXED_DELAY_MS(OhciDevPtr, 10)
        }  
        if (Pstatus->Field.PortResetStatusChange == PORT_RESET_COMPLETE){
    ...
    //Patch work start here
    if (PortFeature == EfiUsbPortEnableChange)
    {
        // Enable the Port here
        OhciHcEnableRootHub( OhciDevPtr, PortNumber );
    }
    //Patch work end here
    ...
    IN UINT8        PortNumber )
{
    OHCI_HC_REGISTERS   *OhciHcReg =
        (OHCI_HC_REGISTERS *) OhciDevPtr->UsbHostControllerBaseAddress;
    volatile HC_RH_PORT_STATUS *Pstatus = &OhciHcReg->HcRhPortStatus[PortNumber];
    ...
    OUT UINT32                     *TransferResult )
{
    ...
    *(UINT32 *) fpData = (UINT32) DataBuffer;
    ...
    WordData = (UINT16) OhciDevPtr->bEndpointSpeed;       // 00/01/10 for HI/LO/FULL
    ...
    Status = OhciHcWaitForTransferComplete( OhciDevPtr,
