    ...
    OUT EFI_USB_PORT_STATUS        *PortStatus )
{
    USB3_CONTROLLER *Usb3Hc = PEI_RECOVERY_USB_XHCI_DEV_FROM_THIS(This);
    volatile XHCI_PORTSC *PortSC;
    UINT32  CurrentPortOffset;
    UINT32  i;
    UINT32  PortStCtl;
    EFI_USB_PORT_STATUS RootPort = {0, 0};

    // Find the proper MMIO access offset for a given port
    for (i=0, CurrentPortOffset=0; i<PortNumber; i++, CurrentPortOffset += 0x10) {}
    ...
    if (PortSC->Field.Ccs != 0) {
        RootPort.PortStatus |= USB_PORT_STAT_CONNECTION;
    }

    if (PortSC->Field.Csc != 0) {
        RootPort.PortChangeStatus |= USB_PORT_STAT_C_CONNECTION;
        // Clear all status change bits
        PortStCtl = PortSC->AllBits;
        PortSC->AllBits = PortStCtl & ~XHCI_PCS_PED;  // DO NOT TOUCH PED
    }

    UpdatePortStatusSpeed(PeiServices, PortSC->Field.PortSpeed, &RootPort.PortStatus);

    // If port is not connected or there was no status change, just return
    if (!(RootPort.PortStatus & USB_PORT_STAT_CONNECTION) ||
        !(RootPort.PortChangeStatus & USB_PORT_STAT_C_CONNECTION)) {
        *PortStatus = RootPort;
        return EFI_SUCCESS;
    }

    // See if port is enabled, if not try to reset it
    if (PortSC->Field.Ped == 0) {

        // Detect and initialize new device. Section 4.3.
        // Reset port and wait for the end of reset TODO:: Remove infinite loop
        PortSC->Field.Pr = 1;
        while (PortSC->Field.Prc == 0) {}

        // After a short delay, check the connect status again, SS device that was
        // originally connected to HS port might get reconnected to the SS port...
        // In this case we remove connect indication from PortStatus and return.
        XHCI_FIXED_DELAY_MS(Usb3Hc, XHCI_SWITCH2SS_DELAY_MS);
        if (PortSC->Field.Ccs == 0) {
            RootPort.PortStatus &= ~USB_PORT_STAT_CONNECTION;
        } else {
            UpdatePortStatusSpeed(PeiServices, PortSC->Field.PortSpeed, &RootPort.PortStatus);
		...
		// Clear all status change bits
		PortSC->AllBits = PortSC->AllBits & ~XHCI_PCS_PED;
    }

    PEI_TRACE((-1, PeiServices, "XhciHcGetRootHubPortStatus for p%d: %x\n", PortNumber, RootPort.PortStatus));
