...
)
{
    ...
    if(EFI_ERROR(Status)){
        DEBUG((-1, "gTpmDevicePpiguid NOT found %r \n", Status)); 
        return Status;
    }
    ...
    if(EFI_ERROR(Status)){
        DEBUG((-1, "gTcgPpiguid NOT found %r \n", Status)); 
        return Status;
    }
    ...
    IN EFI_BOOT_MODE    BootMode )
{
    ...
    {
        DEBUG((-1, "Tcg Startup: returning device error\n"));
    ...
    if(retHeader.RetCode != 0){
    	...
    	DEBUG((-1, "Error sending TPM Startup CMD\n"));
    	return EFI_DEVICE_ERROR;
    }
    ...
    IN EFI_PEI_SERVICES **ps )
{
    ...
    DEBUG((-1, "TCG Pei: Self Test : status=%x; RetCode=%x\n", Status,
            ...
            if ( status == EFI_NOT_READY )
                {
                    ...
                    if ( times > 0x2 )
                    {
                        DEBUG((-1,"xTcgPei::Could not measure PC_CLIENTSPECID \n"));
                        goto M_TRY_DONE;
                    }
                    times++;
                    goto M_TRY_AGAIN;
...
{ 
    ...
    DEBUG((-1, "TCG Pei: measure FwMain: at %x size %d\n", fwv,
               fwv->FvLength));
    ...
    IN EFI_FIRMWARE_VOLUME_HEADER *fwv )
{
    ...
    SHA1HashAll( Context, fwv, (UINTN)fwv->FvLength, (UINT8 *)&TempDigest );
