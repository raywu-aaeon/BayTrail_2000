  ...
  )
{
  ...
  //
  {
  // If one of the parent devices in the device path already supports MNP, 
  // we are dealing with a virtual child produced by the UefiPxeBcDxe driver.
    EFI_DEVICE_PATH_PROTOCOL *DevicePath = DevicePathFromHandle(ControllerHandle);
    EFI_HANDLE Handle;
    if (DevicePath != NULL){
      Status = gBS->LocateDevicePath(&gEfiManagedNetworkServiceBindingProtocolGuid, &DevicePath, &Handle);
  	if (!EFI_ERROR (Status)) {
  		DEBUG((-1,"Returning EFI_UNSUPPORTED\n"));
  ...
  )
{
  ...
  if (Status != EFI_SUCCESS) {
    //
    //default is to disable NetworkStack(both pxe and uefi). If default changes need to change this.
    //
    return Status; 
  }
  ...
  //
  // AMI PORTING END : Check whether Network Stack is enabled in setup or not. If it is disabled return EFI_UNSUPPORTED.
  //
