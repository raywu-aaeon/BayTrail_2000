...
AMI_IP_AVAIL_PROTOCOL     *gAmiIpAvailProtocol=NULL;          // AMI PORTING.
...
//  BOOLEAN     IsIp6Started = FALSE;

 
  // Check for Simple Network Protocol first
  Status = gBS->OpenProtocol (
                    ControllerHandle,
  ...
  //
  // If IPv4 driver is launched & TCP4 is not started then try to install Tcp4ServiceBinding Protocol
  //
  if (gAmiIpAvailProtocol->IPv4_Avail) {        // AMI PORTING. 
    //
...
//  BOOLEAN     IsTcp4Started;
  ...
  //
  // If IPv6 driver is launched & TCP6 is not started then try to install Tcp6ServiceBinding Protocol
  //
  if (gAmiIpAvailProtocol->IPv6_Avail) {              // AMI PORTING.
