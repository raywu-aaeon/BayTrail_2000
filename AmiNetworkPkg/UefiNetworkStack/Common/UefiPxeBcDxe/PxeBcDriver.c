...
NETWORK_STACK       NetworkStackSetupData;                     // AMI PORTING.
  ...
  )
{
  ...
  if (Status != EFI_SUCCESS) {
    //
    // Default is to enable PXE with both IPv4 and Ipv6 and 
    // Set PxeBootWaitTime as zero.
    //
  ...
  )
{
  ...
  if (IpVersion == IP_VERSION_4) {
    //
	// Check whether PxeIpv4 is disabled in setup if so return EFI_UNSUPPORTED.
	//
  ...
  )
{
  ...
  if (IpVersion == IP_VERSION_4) {
    ...
    //
    if (NetworkStackSetupData.Ipv4Pxe == 1) {       // AMI PORTING.
    ...
    //
    if (NetworkStackSetupData.Ipv6Pxe == 1) {      // AMI PORTING.
