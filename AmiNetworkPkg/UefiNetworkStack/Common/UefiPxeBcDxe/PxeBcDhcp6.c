  ...
  )
{
  ...
  //EFI_PXE_BASE_CODE_MODE              *Mode;    AMI PORTING : Remove unused Variable.
  ...
  PxeBc       = &Private->PxeBc;
  //Mode        = PxeBc->Mode;                      AMI PORTING : Remove unused Variable.
  ...
  )
{
  ...
  case Dhcp6SendSolicit:
    ...
    //
    CopyMem (&Mode->DhcpDiscover.Dhcpv6, &Packet->Dhcp6, Packet->Length);       // AMI PORTING.
    break;
