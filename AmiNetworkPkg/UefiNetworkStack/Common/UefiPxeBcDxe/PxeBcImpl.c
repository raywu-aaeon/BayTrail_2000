...
//
UINTN   TftpBlockSize = 0;
//
// AMI PORTING END : Remove PCD.
//

//
// AMI PORTING START : Added to support PXE abort using ESC key.
//
//
...
// AMI PORTING START.
//
extern NETWORK_STACK    NetworkStackSetupData;
//
// AMI PORTING END.
//
...
//<AMI_PHDR_START>
//----------------------------------------------------------------------------
// Procedure: HandleEscKeytoAbortPxeBoot
//
// Description:
...
// Output:
//  EFI_STATUS
//      EFI_SUCCESS - If ESC key is not pressed with in given time.
//      EFI_ABORTED - If ESC key is pressed with in given time.
//
//
//----------------------------------------------------------------------------
//<AMI_PHDR_END>
  ...
  )
{
  ...
  if (Mode->UsingIpv6) {
    ...
    if ( NetworkStackSetupData.PxeBootWaitTime ) {
        //
        // Handle ESC key to abort PXE boot within wait time.
        //
        Status = HandleEscKeytoAbortPxeBoot ( NetworkStackSetupData.PxeBootWaitTime );
        if ( EFI_ERROR (Status) ) {
            return Status;
        }
    } // if (NetworkStackSetupData.PxeBootWaitTime)
  ...
  )
{
  ...
  //
  //
  // AMI PORTING START : Add the console status messages.
  //
  ...
  if (!EFI_ERROR (Status)) AsciiPrint ("\n>>Media Present......");
  //
  // AMI PORTING END : Add the console status messages.
  //
