  ...
  )
{
  ...
  if (Status == EFI_NOT_FOUND) {
    ...
    //
    // For 1st boot, randomly generated IAID in both Pxe and Ipv6 drivers has same value as it is generated in the same time frame for the same NIC.
    // When Pxe tries to configure DHCP with same IAID, it's getting failed as DHCP has already got configured with this IAID by IPv4 driver.
    // Changing the logic to generate different IAID for Ipv6.
    //
    Instance->IaId ^= 0x1;      // AMI PORTING.
...
//  Ip6ConfigFormUnload (Instance);                  // AMI PORTING.
