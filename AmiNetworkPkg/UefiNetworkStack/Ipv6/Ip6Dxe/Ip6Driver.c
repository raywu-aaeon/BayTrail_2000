...
// AMI PORTING START.
//
AMI_IP_AVAIL_PROTOCOL gAmiIpAvailProtocolInst = {
// IPv4,  IPv6,       Reserved
 IPVX_NA, IPVX_AVAIL, 0
};
//
// AMI PORTING END.
//
  ...
  )
{
  ...
  //
  EFI_STATUS                Status;
  EFI_GUID                  AmiIpAvailProtocolGuid = AMI_IP_AVAIL_PROTOCOL_GUID;
  AMI_IP_AVAIL_PROTOCOL     *AmiIpAvailProtocol;
  EFI_HANDLE                Handle;

  //
  // Identify whether AMI_IP_AVAIL_PROTOCOL is already installed by IP4 driver
  //
  Status = gBS->LocateProtocol (
                 &AmiIpAvailProtocolGuid,
                 NULL,
                 (void **)&AmiIpAvailProtocol    
                 );

  if (EFI_ERROR(Status)) {
    //
    // AMI_IP_AVAIL_PROTOCOL is not installed so publish new one with IPv6 availability status set
    //
    Handle = NULL;
    Status = gBS->InstallProtocolInterface (
                   ...
                   &gAmiIpAvailProtocolInst
                   );
  } else {

    //
    // AMI_IP_AVAIL_PROTOCOL is already installed so update IPv6 availability status 
    // in the existing Protocol
    //
    AmiIpAvailProtocol->IPv6_Avail = IPVX_AVAIL;
  }
  //
  // AMI PORTING END.
  //
...
{
  EFI_STATUS                Status;                 // AMI PORTING.
  //
  // Test for the MNP service binding Protocol
  //
  //
  // AMI PORTING START : Check the status of MNP Service binding protocol before procedding further.
  //
  ...
  //
  // AMI PORTING END : Check the status of MNP Service binding protocol before procedding further.
  //
  
  //
  // AMI PORTING START : Check whether ARP service binding protocol is available.
  //
  //
  ...
  Status = gBS->OpenProtocol (
   ...
   return Status;
   //
  // AMI PORTING END : Check whether ARP service binding protocol is available.
  //
}
