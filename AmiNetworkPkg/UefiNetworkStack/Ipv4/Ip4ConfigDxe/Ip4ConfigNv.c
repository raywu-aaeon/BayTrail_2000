...
//IP4_CONFIG_IFR_NVDATA            *IfrFormNvData;          // AMI PORTING : (EIP82809)
  ...
  UINTN                            Size;
  UINTN                            BufferSize;
  UINTN                            NicItemConfigSize;       // AMI PORTING : (EIP82809)
  UINTN                            MaxIp4ConfigInVariable;  // AMI PORTING : (EIP82809)
...
// AMI PORTING START : (EIP82809>)
    //Edk106 Shell 1.0 IfConfig.inf        : MAX_IP4_CONFIG_IN_VARIABLE = 128
    //UDK2010.SR1 Shell 1.0 EdkShellBinPkg : MAX_IP4_CONFIG_IN_VARIABLE = 16
    //UDK2010.SR1 Shell 2.0 ShellBinPkg    : MAX_IP4_CONFIG_IN_VARIABLE = 16

    StrPointer = StrStr (Request, L"&WIDTH=");
    if ((Request == NULL) || (StrPointer == NULL)) {
      return EFI_NOT_FOUND;
    }
    Size = StrLen(L"&WIDTH=");
    NicItemConfigSize = 0;
    NicItemConfigSize = StrHexToUintn(StrPointer+Size);
    if(NicItemConfigSize == 0) {
      return EFI_NOT_FOUND;
    }
    if(NicItemConfigSize < sizeof(NIC_IP4_CONFIG_INFO)){
      return EFI_NOT_FOUND;
    }
    MaxIp4ConfigInVariable = NicItemConfigSize - sizeof(NIC_IP4_CONFIG_INFO);
    if( (MaxIp4ConfigInVariable % sizeof(EFI_IP4_ROUTE_TABLE)) != 0){
      return EFI_NOT_FOUND;
    }
    MaxIp4ConfigInVariable = MaxIp4ConfigInVariable / sizeof(EFI_IP4_ROUTE_TABLE);
    if( MaxIp4ConfigInVariable > 128){
      return EFI_NOT_FOUND;
    }
// AMI PORTING END : (EIP82809<)

//  IfrDeviceNvData = AllocateZeroPool (NIC_ITEM_CONFIG_SIZE);  // AMI PORTING : (EIP82809-)
    IfrDeviceNvData = AllocateZeroPool (NicItemConfigSize);     // AMI PORTING : (EIP82809)
...
//    BufferSize = NIC_ITEM_CONFIG_SIZE;    // AMI PORTING : (EIP82809-)
      BufferSize = NicItemConfigSize;       // AMI PORTING : (EIP82809)
...
// AMI PORTING START : (EIP82809>)
  UINTN                            Size;
  CHAR16                          *StrPointer;
  UINTN                            NicItemConfigSize;
  UINTN                            MaxIp4ConfigInVariable;
// AMI PORTING END : (EIP82809<)
  ...
  //
//Ip4ConfigReclaimVariable ();	// AMI PORTING : (EIP82809-)
...
/* AMI PORTING START : (EIP82809)
  ...
  } else if (HiiIsConfigHdrMatch (Configuration, &gEfiNicIp4ConfigVariableGuid, EFI_NIC_IP4_CONFIG_VARIABLE)) {  //(EIP82809)
*/// AMI PORTING END : (EIP82809)
...
// AMI PORTING START : (EIP82809>)
  if (HiiIsConfigHdrMatch (Configuration, &gEfiNicIp4ConfigVariableGuid, EFI_NIC_IP4_CONFIG_VARIABLE)) {
    ...
    StrPointer = StrStr (Configuration, L"&WIDTH=");
    if ((Configuration == NULL) || (StrPointer == NULL)) {
      return EFI_NOT_FOUND;
    }
    Size = StrLen(L"&WIDTH=");
    NicItemConfigSize = 0;
...
//ConfigAccess->Callback      = Ip4FormCallback;    // AMI PORTING : (EIP82809-)
  ConfigAccess->Callback      = NULL;               // AMI PORTING : (EIP82809)
...
{    // AMI PORTING START : (EIP82809>)
  ...
  UINT32                           Length;
  UINT8                            *Data08Point1;
  EFI_HII_HANDLE                   OutHiiHandle;

  EFI_HII_PACKAGE_HEADER           EndOfPakageList = { sizeof (EFI_HII_PACKAGE_HEADER),
                                                       EFI_HII_PACKAGE_END
                                                     };
  ...
  CopyGuid(&PackageListHeader->PackageListGuid, &gNicIp4ConfigNvDataGuid);
  PackageListHeader->PackageLength = Length;

  Data08Point1 = (UINT8*)(PackageListHeader + 1);
  CopyMem (Data08Point1, &EndOfPakageList, sizeof (EndOfPakageList));
  ...
  Instance->RegisteredHandle = OutHiiHandle;
  ...
  )
{
  ...
  //
//  HiiRemovePackages (Instance->RegisteredHandle);    // AMI PORTING : (EIP82809-)
// AMI PORTING START : (EIP82809>)
  if(Instance->RegisteredHandle != NULL){
