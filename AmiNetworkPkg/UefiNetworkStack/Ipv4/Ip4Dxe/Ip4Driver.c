  ...
  )
{
  ...
  //
  // Identify whether AMI_IP_AVAIL_PROTOCOL is already installed by IP6 driver
  //
  Status = gBS->LocateProtocol (
                 &AmiIpAvailProtocolGuid,
                 NULL,
                 (void**)&AmiIpAvailProtocol    // AMI PORTING : Added to resolve build error with GCC compiler.
                 );

  if (EFI_ERROR(Status)) {
    //
    // AMI_IP_AVAIL_PROTOCOL is not installed so publish new one with IPv4 availability status set
    //
    Handle = NULL;
    ...
    //
    // AMI_IP_AVAIL_PROTOCOL is already installed so update IPv4 availability status 
    // in the existing Protocol
    //
    AmiIpAvailProtocol->IPv4_Avail = IPVX_AVAIL;
  }
  //
  // AMI PORTING END.
  //
